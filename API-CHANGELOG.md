...
- `sign`, `sign_for`, `submit`, `submit_multisigned`, `simulate`, `path_find`, `ripple_path_find`: An issued currency `value` whose digit string exceeds 2^63 - 1, which trailing zeros are enough to reach, no longer wraps into an unrelated number. A `TrustSet` with a `LimitAmount` of `"-1000.0000000000000000"` was accepted with a limit of `844.6744073709552`, while `"-1000"` was rejected with `temBAD_LIMIT`. Digits beyond the 16 an issued currency holds are rounded, as they already were for shorter values, and a value too large to represent returns an error. [#8190](https://github.com/XRPLF/rippled/pull/8190)
