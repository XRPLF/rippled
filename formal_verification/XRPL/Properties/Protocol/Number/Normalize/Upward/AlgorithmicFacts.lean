...
/-! # Algorithmic decomposition for `Number.normalize` (`.upward`)
...
`g0_represents_zero`. The `.upward` mode can round up, so the floor constraint
...
When `n.normalize largeRange.min largeRange.max .upward = .ok result` with
...
theorem normalize_algorithmic_facts_upward (n result : Number)
  ...
  · push_neg at hzero
    ...
    simp only at hok
    obtain ⟨res, hrup⟩ : ∃ res,
        g3.doRoundUp n.negative_ m3 e3 largeRange.min largeRange.max .upward "Number::normalize 2" = .ok res := by
      match hrup : g3.doRoundUp n.negative_ m3 e3 largeRange.min largeRange.max .upward "Number::normalize 2" with
    ...
    have hrup_pos : g3.doRoundUp false m3 e3 largeRange.min largeRange.max .upward "Number::normalize 2"
        = .ok res_pos :=
      doRoundUp_false_from_ok g3 n.negative_ m3 e3 .upward "Number::normalize 2" res hrup
