...
theorem normalize_rounds_downward (n result : Number)
    ...
    Rounds result n.toRat .downward (10 / (2 ^ 63 + 2 : ℚ)) := by
  obtain ⟨h_dir, h_mag⟩ :=
    normalize_rounding_bound_downward n result hn_mant_ne hok hresult
  exact ⟨h_dir, le_of_lt h_mag⟩

/-- The `10/(2^63+2)` downward normalize bound is a **tight supremum but not
    attained**: normalizing the positive value `2^63+1` truncates toward zero by
    exactly 9 ULP-at-scale, giving relative error `9/(2^63+1)`, which strictly
    exceeds `9/(2^63+2)`. So no numerator below 10 suffices, yet 10 is never
    reached (since `f < 1`). -/
