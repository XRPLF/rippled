...
`m ≤ maxRep`, pushing each dropped digit into the guard. -/
...
when the output mantissa is `(maxRep + 1) / 10`: the leading dropped digit
must be `≥ 8`, which rules out `f = 1/2` at the floor mantissa value. -/
theorem scaleDown128_correct :
  ...
  | case1 M e g0 hcond d IH =>
    ...
    have hEuc : M.toNat = 10 * (M / 10).toNat + M.toNat % 10 := by
      rw [hM10_nat]; omega
    have hM_gt_maxRep : M.toNat > maxRep.toNat := by
    ...
    · rintro ⟨_hk_pos, hm_floor⟩
      ...
      · subst hk0
        ...
        have hmaxRep_eq : maxRep.toNat = maxRepNat := by decide
        have : 9223372036854775800 + M.toNat % 10 > maxRepNat := by
          have := hEuc
          have : M.toNat = 10 * mantissaFloor + M.toNat % 10 := by
            rw [← h_div_val]; linarith
          rw [hmaxRep_eq] at hM_gt_maxRep; omega
  ...
  | case2 M e g0 hcond =>
    intro f0 hrep0
    have hrep_le : M ≤ toUInt128 maxRep := BitVec.not_lt.mp hcond
    have hexit : scaleDown128 M e g0 = (toUInt64 M, e, g0) := by
      unfold scaleDown128
      simp [hcond]
    rw [hexit]
    have hM_le_rep : M.toNat ≤ maxRep.toNat := by
...
/-- Lower bound on scaleDown128 output: `m.toNat ≥ (maxRep.toNat + 1) / 10`. -/
theorem scaleDown128_lower_bound :
    ∀ (M : UInt128) (e : Int) (g0 : Guard),
      M.toNat > maxRep.toNat →
      let (m, _, _) := scaleDown128 M e g0
      m.toNat ≥ (maxRep.toNat + 1) / 10 := by
  intro M e g0
  induction M, e, g0 using scaleDown128.induct with
  | case1 M e g0 hcond d IH =>
    intro _hM_gt
    ...
    have hM10_nat : (M / 10 : UInt128).toNat = M.toNat / 10 := by
      rw [BitVec.toNat_udiv, h10_128]
    by_cases h_next_gt : (M / 10 : UInt128).toNat > maxRep.toNat
    ...
    · push_neg at h_next_gt
      have h_div_ge : M.toNat / 10 ≥ (maxRep.toNat + 1) / 10 :=
        Nat.div_le_div_right _hM_gt
      unfold scaleDown128
      simp only [hcond]
      unfold scaleDown128
      have h_next_cond :
          ¬ ((M / 10 : UInt128) > toUInt128 maxRep) := by
        intro h_gt_rep
      ...
      have h_fit : (M / 10 : UInt128).toNat < 2 ^ 64 := by
        have : maxRep.toNat < 2 ^ 64 := maxRep.toNat_lt
        omega
      simp only [↓reduceDIte]
      change (toUInt64 (M / 10)).toNat ≥ (maxRep.toNat + 1) / 10
