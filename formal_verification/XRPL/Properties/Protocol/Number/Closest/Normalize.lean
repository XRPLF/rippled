...
import XRPL.Properties.Protocol.Number.Closest.Defs
...
/-- Construct a UInt64 from a Nat with proof. -/
private def natToUInt64 (m : ℕ) (_h : m < 2 ^ 64) : UInt64 := UInt64.ofNat m

private lemma natToUInt64_toNat (m : ℕ) (h : m < 2 ^ 64) : (natToUInt64 m h).toNat = m := by
  unfold natToUInt64
...
/-- Helper: an upperPosAux/lowerPosAux output is normalized given valid mantissa. -/
private lemma isNormalized_of_valid_mantissa
    (neg : Bool) (m : ℕ) (e : Int)
    ...
    (h_m64 : m < 2 ^ 64) :
    (⟨neg, natToUInt64 m h_m64, e⟩ : Number).isNormalized := by
  right
  refine ⟨?_, ?_, ?_, h_emin, h_emax⟩
  · rw [UInt64.le_iff_toNat_le]
    rw [natToUInt64_toNat]
    exact h_min
  · rw [UInt64.le_iff_toNat_le]
    rw [natToUInt64_toNat]
    exact h_max
