...
import XRPL.Properties.Protocol.Number.Closest.Tightness
...
/-- `bumpToValidMantissa m_ceil - truncToValidMantissa m_floor ≤ 10`
when `m_floor, m_ceil` differ by at most 1 and both sit in `[10^18, 10^19)`. -/
private lemma bump_sub_trunc_le (m_floor m_ceil : ℕ)
    (h_close : m_ceil = m_floor ∨ m_ceil = m_floor + 1)
    (h_hi : m_ceil < 10 ^ 19)
    (m_b : ℕ) (h_b : bumpToValidMantissa m_ceil = some m_b) :
    m_b ≤ truncToValidMantissa m_floor + 10 := by
  have h_maxR : maxRep.toNat = maxRepNat := by decide
  have h_cusp : cuspMin = maxRepCuspTarget := by decide
  by_cases h_floor_le : m_floor ≤ maxRep.toNat
  · have h_trunc_eq : truncToValidMantissa m_floor = m_floor := by
      unfold truncToValidMantissa; rw [if_pos h_floor_le]
    rw [h_trunc_eq]
    rcases h_close with h_eq | h_succ
    · rw [h_eq] at h_b
      have h_bump_eq : bumpToValidMantissa m_floor = some m_floor := by
        unfold bumpToValidMantissa; rw [if_pos h_floor_le]
      rw [h_bump_eq] at h_b
      have h_mb : m_b = m_floor := (Option.some.inj h_b).symm
      omega
    · by_cases h_ceil_le : m_ceil ≤ maxRep.toNat
      · have h_bump_eq : bumpToValidMantissa m_ceil = some m_ceil := by
          unfold bumpToValidMantissa; rw [if_pos h_ceil_le]
        rw [h_bump_eq] at h_b
        have h_mb : m_b = m_ceil := (Option.some.inj h_b).symm
        omega
      · -- m_floor = maxRep, m_ceil = maxRep + 1 (cusp boundary)
        push_neg at h_ceil_le
        have h_floor_eq : m_floor = maxRep.toNat := by omega
        have h_ceil_v : m_ceil = maxRep.toNat + 1 := by omega
        have h_in_gap : m_ceil < cuspMin := by omega
        have h_bump_eq : bumpToValidMantissa m_ceil = some cuspMin := by
          unfold bumpToValidMantissa
          rw [if_neg (by omega), if_pos h_in_gap]
        rw [h_bump_eq] at h_b
        have h_mb : m_b = cuspMin := (Option.some.inj h_b).symm
        rw [h_mb, h_floor_eq]
        omega
  · push_neg at h_floor_le
    by_cases h_floor_in_gap : m_floor < cuspMin
    · have h_trunc_eq : truncToValidMantissa m_floor = maxRep.toNat := by
        unfold truncToValidMantissa
        rw [if_neg (by omega), if_pos h_floor_in_gap]
      rw [h_trunc_eq]
      have h_ceil_gt : m_ceil > maxRep.toNat := by
        rcases h_close with h_eq | h_succ
        · omega
        · omega
      by_cases h_ceil_in_gap : m_ceil < cuspMin
      · have h_bump_eq : bumpToValidMantissa m_ceil = some cuspMin := by
          unfold bumpToValidMantissa
          rw [if_neg (by omega), if_pos h_ceil_in_gap]
        rw [h_bump_eq] at h_b
        have h_mb : m_b = cuspMin := (Option.some.inj h_b).symm
        omega
      · push_neg at h_ceil_in_gap
        have h_ceil_eq_cusp : m_ceil = cuspMin := by
          rcases h_close with h_eq | h_succ
          · omega
          · omega
        have h_rounded : ((m_ceil + 9) / 10) * 10 = cuspMin := by
          rw [h_ceil_eq_cusp]; decide
        have h_lt_pow19 : ((m_ceil + 9) / 10) * 10 < 10^19 := by
          rw [h_rounded]; decide
        have h_bump_eq : bumpToValidMantissa m_ceil = some (((m_ceil + 9) / 10) * 10) := by
          unfold bumpToValidMantissa
          rw [if_neg (by omega), if_neg (by omega), if_pos h_lt_pow19]
        rw [h_bump_eq] at h_b
        have h_mb : m_b = ((m_ceil + 9) / 10) * 10 := (Option.some.inj h_b).symm
        rw [h_mb, h_rounded]
        omega
    · push_neg at h_floor_in_gap
      have h_trunc_eq : truncToValidMantissa m_floor = (m_floor / 10) * 10 := by
        unfold truncToValidMantissa
        rw [if_neg (by omega), if_neg (by omega)]
      rw [h_trunc_eq]
      have h_ceil_ge_cusp : m_ceil ≥ cuspMin := by
        rcases h_close with h_eq | h_succ
        · omega
        · omega
      have h_lt_pow19 : ((m_ceil + 9) / 10) * 10 < 10^19 := by
        by_contra h_not
        push_neg at h_not
        have h_bump_none : bumpToValidMantissa m_ceil = none := by
          unfold bumpToValidMantissa
          rw [if_neg (by omega : ¬ m_ceil ≤ maxRep.toNat),
              if_neg (by omega : ¬ m_ceil < cuspMin),
              if_neg (by omega : ¬ ((m_ceil + 9) / 10) * 10 < 10^19)]
        rw [h_bump_none] at h_b
        exact absurd h_b (by simp)
      have h_bump_eq : bumpToValidMantissa m_ceil = some (((m_ceil + 9) / 10) * 10) := by
        unfold bumpToValidMantissa
        rw [if_neg (by omega), if_neg (by omega), if_pos h_lt_pow19]
      rw [h_bump_eq] at h_b
      have h_mb : m_b = ((m_ceil + 9) / 10) * 10 := (Option.some.inj h_b).symm
      rw [h_mb]
      have h_ceil_le_floor_succ : m_ceil ≤ m_floor + 1 := by
        rcases h_close with h_eq | h_succ
        · omega
        · omega
      have h_div_le : (m_ceil + 9) / 10 ≤ (m_floor + 10) / 10 := by
        apply Nat.div_le_div_right; omega
      have h_div_eq : (m_floor + 10) / 10 = m_floor / 10 + 1 := by
        have := Nat.add_div_right m_floor (by norm_num : 0 < 10)
        omega
      omega
...
theorem no_normalized_in_open_ulp_gap_pos
  ...
  have h_neg_false : n.negative_ = false := by
    ...
    · exfalso
      ...
      linarith
  have h_m_ne : n.mantissa_ ≠ 0 := by
    intro h_eq
    rw [Number.toRat_of_nonneg n h_neg_false] at h_n_pos
    have h_m_zero : n.mantissa_.toNat = 0 := by rw [h_eq]; rfl
    rw [h_m_zero] at h_n_pos
    have h_lhs_zero : ((0 : ℕ) : ℚ) * (10 : ℚ) ^ n.exponent_ = 0 := by ring
    linarith [h_lhs_zero]
...
/-- Negative analog of `no_normalized_in_open_ulp_gap_pos`. -/
theorem no_normalized_in_open_ulp_gap_neg
    (e : Int) (k : ℕ)
    (h_e_min : minExponent ≤ e) (h_e_max : e ≤ maxExponent)
    (h_k_min : (10 : ℕ) ^ 18 ≤ k) (h_k_max : k < (10 : ℕ) ^ 19)
    (n : Number) (h_norm : n.isNormalized)
    (h_n_neg : n.toRat < 0)
  ...
  exact no_normalized_in_open_ulp_gap_pos e k h_e_min h_e_max h_k_min h_k_max n' h_n'_norm
