...
/-- Under `operator_mul`'s hypotheses with `.upward`, the result is normalized. -/
theorem operator_mul_result_isNormalized_upward (x y result : Number)
  ...
  have h_rup_exists : ∃ res : RoundResult,
      g.doRoundUp zn zm ze' largeRange.min largeRange.max .upward "Number::multiplication overflow" = .ok res := by
    match hg : g.doRoundUp zn zm ze' largeRange.min largeRange.max .upward "Number::multiplication overflow" with
  ...
  have h_inv := doRoundUp_output_invariants_upward g zn zm ze' hzm_ge hzm_le_maxRep "Number::multiplication overflow" res h_rup hres_mant_ne
...
/-- Under `operator_mul`'s hypotheses with `.upward`, max-exponent implies mantissa in range. -/
theorem operator_mul_no_overflow_mantissa_upward (x y result : Number)
    (hx : x.isNormalized) (hy : y.isNormalized)
    (hx_mant_ne : x.mantissa_ ≠ 0) (hy_mant_ne : y.mantissa_ ≠ 0)
    (hok : Number.operator_mul x y .upward = .ok result)
    (hresult : result.mantissa_ ≠ 0)
...
theorem operator_mul_algorithmic_facts_upward (x y result : Number)
    ...
    ∃ (zm : UInt64) (ze' : Int) (f : ℚ) (g : Guard) (res_pos : RoundResult),
      ...
      g.doRoundUp false zm ze' largeRange.min largeRange.max .upward "Number::multiplication overflow" = .ok res_pos ∧
