...
/-- Under `operator_mul`'s hypotheses with `.to_nearest`, the result is normalized. -/
theorem operator_mul_result_isNormalized (x y result : Number)
  ...
  have h_rup_exists : ∃ res : RoundResult,
      g.doRoundUp zn zm ze' largeRange.min largeRange.max .to_nearest "Number::multiplication overflow" = .ok res := by
    match hg : g.doRoundUp zn zm ze' largeRange.min largeRange.max .to_nearest "Number::multiplication overflow" with
  ...
  have h_inv := doRoundUp_output_invariants g zn zm ze' hzm_ge hzm_le_maxRep "Number::multiplication overflow" res h_rup hres_mant_ne
...
/-- Under operator_mul's hypotheses, if `result.exponent_ ≥ maxExponent` then
`result.mantissa_ ≤ maxRep`. Algorithmic no-overflow contract derived from `hok`. -/
theorem operator_mul_no_overflow_mantissa (x y result : Number)
    (hx : x.isNormalized) (hy : y.isNormalized)
    (hx_mant_ne : x.mantissa_ ≠ 0) (hy_mant_ne : y.mantissa_ ≠ 0)
    (hok : Number.operator_mul x y .to_nearest = .ok result)
    (hresult : result.mantissa_ ≠ 0)
...
theorem operator_mul_algorithmic_facts (x y result : Number)
    (hx : x.isNormalized) (hy : y.isNormalized)
    (hx_mant_ne : x.mantissa_ ≠ 0) (hy_mant_ne : y.mantissa_ ≠ 0)
    (hok : Number.operator_mul x y .to_nearest = .ok result)
    (hresult : result.mantissa_ ≠ 0) :
    ∃ (zm : UInt64) (ze' : Int) (f : ℚ) (g : Guard) (res_pos : RoundResult),
      ...
      g.doRoundUp false zm ze' largeRange.min largeRange.max .to_nearest "Number::multiplication overflow" = .ok res_pos ∧
  ...
  -- Extract the doRoundUp result for the zn case (used for normalize)
  have h_rup_exists : ∃ res : RoundResult,
      g.doRoundUp zn zm ze' largeRange.min largeRange.max .to_nearest "Number::multiplication overflow" = .ok res := by
    match hg : g.doRoundUp zn zm ze' largeRange.min largeRange.max .to_nearest "Number::multiplication overflow" with
    | .error e => simp only [hg, reduceCtorEq] at hok
    | .ok r => exact ⟨r, rfl⟩
  obtain ⟨res, h_rup⟩ := h_rup_exists
  -- Extract the doRoundUp result for the false (positive) case
  simp only [h_rup] at hok
  have hres_mant_ne : res.mantissa_ ≠ 0 :=
    Number.normalize_mantissa_ne_zero_of_result hok hresult
  have h_inv := doRoundUp_output_invariants g zn zm ze' hzm_ge hzm_le_maxRep "Number::multiplication overflow" res h_rup hres_mant_ne
  obtain ⟨h_res_min, h_res_max, h_res_exp, h_res_mod⟩ := h_inv
  -- The false-case doRoundUp: mantissa/exponent same as zn case, only negative_ differs
  -- Since res.exponent_ ≤ maxExponent (no overflow), false case also gives .ok
