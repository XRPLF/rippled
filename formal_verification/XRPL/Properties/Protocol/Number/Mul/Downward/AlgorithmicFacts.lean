...
/-- Under `operator_mul`'s hypotheses with `.downward`, the result is normalized. -/
theorem operator_mul_result_isNormalized_downward (x y result : Number)
  ...
  have h_rup_exists : ∃ res : RoundResult,
      g.doRoundUp zn zm ze' largeRange.min largeRange.max .downward "Number::multiplication overflow" = .ok res := by
    match hg : g.doRoundUp zn zm ze' largeRange.min largeRange.max .downward "Number::multiplication overflow" with
  ...
  have h_inv := doRoundUp_output_invariants_downward g zn zm ze' hzm_ge hzm_le_maxRep "Number::multiplication overflow" res h_rup hres_mant_ne
...
/-- Under `operator_mul`'s hypotheses with `.downward`, max-exponent implies mantissa in range. -/
theorem operator_mul_no_overflow_mantissa_downward (x y result : Number)
    (hx : x.isNormalized) (hy : y.isNormalized)
    (hx_mant_ne : x.mantissa_ ≠ 0) (hy_mant_ne : y.mantissa_ ≠ 0)
    (hok : Number.operator_mul x y .downward = .ok result)
    (hresult : result.mantissa_ ≠ 0)
...
private lemma scaleDown128_sbit_preserved
...
theorem operator_mul_algorithmic_facts_downward (x y result : Number)
    ...
    ∃ (zm : UInt64) (ze' : Int) (f : ℚ) (g : Guard) (res_pos : RoundResult),
      ...
      g.doRoundUp false zm ze' largeRange.min largeRange.max .downward "Number::multiplication overflow" = .ok res_pos ∧
      |result.toRat| = (res_pos.mantissa_.toNat : ℚ) * 10 ^ res_pos.exponent_ ∧
      res_pos.mantissa_ ≠ 0 ∧
      represents g f ∧
      result.negative_ = (x.negative_ != y.negative_) ∧
      g.sbit_ = (x.negative_ != y.negative_) := by
  have hx_bounds := hx.mantissaBounds hx_mant_ne
  have hy_bounds := hy.mantissaBounds hy_mant_ne
  have hx_mant_pos : 0 < x.mantissa_.toNat := by
    have : largeRange.min.toNat ≤ x.mantissa_.toNat := UInt64.le_iff_toNat_le.mp hx_bounds.1
  ...
  have h_floor_constraint : zm.toNat = mantissaFloor → (8 : ℚ) / 10 ≤ f := by
    ...
    have hk_pos : k > 0 := by
      ...
      push_neg at h
      interval_cases k
      simp at hzm128_decomp
      omega
    have h_maxRep_floor_eq : (maxRep.toNat + 1) / 10 = mantissaFloor := by
      rw [maxRep_val]
    have h_zm_eq_floor : zm.toNat = (maxRep.toNat + 1) / 10 := by
      rw [h_maxRep_floor_eq]; exact h_zm_floor
    have h_residue_lb : 8 * 10 ^ (k - 1) ≤ zm128.toNat % 10 ^ k :=
      hfloor ⟨hk_pos, h_zm_eq_floor⟩
    rw [hf_def]
    rw [le_div_iff₀ (by positivity : (0 : ℚ) < 10 ^ k)]
    have h_10pow_split : (10 : ℚ) ^ k = 10 * 10 ^ (k - 1) := by
      have hkk : k = (k - 1) + 1 := by omega
      conv_lhs => rw [hkk]
      ring
    rw [h_10pow_split]
    have h_residue_q : ((8 * 10 ^ (k - 1) : ℕ) : ℚ) ≤ ((zm128.toNat % 10 ^ k : ℕ) : ℚ) := by
      exact_mod_cast h_residue_lb
    calc (8 : ℚ) / 10 * (10 * 10 ^ (k - 1))
        = 8 * 10 ^ (k - 1) := by ring
      _ = ((8 * 10 ^ (k - 1) : ℕ) : ℚ) := by push_cast; ring
      _ ≤ ((zm128.toNat % 10 ^ k : ℕ) : ℚ) := h_residue_q
  have h_rup_exists : ∃ res : RoundResult,
      g.doRoundUp zn zm ze' largeRange.min largeRange.max .downward "Number::multiplication overflow" = .ok res := by
    match hg : g.doRoundUp zn zm ze' largeRange.min largeRange.max .downward "Number::multiplication overflow" with
    | .error e => simp only [hg, reduceCtorEq] at hok
  ...
  have h_rup_pos : g.doRoundUp false zm ze' largeRange.min largeRange.max .downward "Number::multiplication overflow" = .ok res_pos :=
    doRoundUp_false_from_ok g zn zm ze' .downward "Number::multiplication overflow" res h_rup
  have h_result_eq_res : result = res.toNumber :=
    Number.normalize_eq_of_invariants_downward h_res_min h_res_max h_res_exp h_res_mod hok
  ...
  have h_g_sbit : g.sbit_ = zn := by
    ...
    rw [h_g_eq, scaleDown128_sbit_preserved]
    rw [hg0_def]
    by_cases hzn : zn
    · rw [if_pos hzn]
      change true = zn
      exact hzn.symm
    · rw [if_neg hzn]
      have hzn_eq : zn = false := Bool.not_eq_true _ |>.mp hzn
      change false = zn
      exact hzn_eq.symm
  refine ⟨zm, ze', f, g, res_pos, hzm_ge, hzm_le_maxRep, hf_nn, hf_lt, h_floor_constraint,
    habs_xy_eq2, h_rup_pos, h_result_abs, hres_pos_mant_ne, hf_rep, ?_, ?_⟩
  · rw [h_res_neg_eq_zn]
  · exact h_g_sbit
