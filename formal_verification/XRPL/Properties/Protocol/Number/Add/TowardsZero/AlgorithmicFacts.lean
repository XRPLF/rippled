...
/-! # Algorithmic decomposition for `operator_add` — same-sign branch (towards_zero) -/

/-- For same-sign Numbers, `|x.toRat + y.toRat| = |x.toRat| + |y.toRat|`. -/
private lemma abs_add_eq_of_same_sign_tz {x y : Number} (h : x.negative_ = y.negative_) :
...
private lemma represents_initial_guard_eq_tz (xn : Bool) :
...
private lemma alignDown_abs_value_tz
...
/-- Helper: post-alignment processing for the same-sign branch (towards_zero). -/
private theorem operator_add_post_alignment_towards_zero
    ...
    (hok : (let zm128 : UInt128 := toUInt128 xm_a + toUInt128 ym_a
            ...
            match p.2.2.doRoundUp xn p.1 p.2.1 largeRange.min largeRange.max
                  .towards_zero "Number::addition overflow" with
            | .error err => Except.error err
            | .ok res => res.toNumber.normalize largeRange.min largeRange.max .towards_zero)
           = .ok result) :
  ...
  · have h_cond_true : (zm128 > toUInt128 largeRange.max
    ...
    have h_rup_exists : ∃ res : RoundResult,
        g_new.doRoundUp xn zm_new (e_common + 1) largeRange.min largeRange.max .towards_zero
          "Number::addition overflow" = .ok res := by
      match hg : g_new.doRoundUp xn zm_new (e_common + 1) largeRange.min largeRange.max .towards_zero "Number::addition overflow" with
    ...
    have h_inv := doRoundUp_output_invariants_towards_zero g_new xn zm_new (e_common + 1)
    ...
    have h_rup_pos : g_new.doRoundUp false zm_new (e_common + 1) largeRange.min largeRange.max .towards_zero "Number::addition overflow" = .ok res_pos :=
      doRoundUp_false_from_ok g_new xn zm_new (e_common + 1) .towards_zero "Number::addition overflow" res h_rup
    ...
    refine ⟨zm_new, e_common + 1, f_new, g_new, res_pos, hzm_new_ge_floor, hzm_new_le_maxRep,
            hf_new_nn, hf_new_lt, h_value_eq, h_rup_pos, h_result_abs,
  ...
  · push_neg at h_drop
    ...
    have hok' : (match g_aln.doRoundUp xn zm_new e_common largeRange.min largeRange.max
      ...
      exact h
    have h_rup_exists : ∃ res : RoundResult,
        g_aln.doRoundUp xn zm_new e_common largeRange.min largeRange.max .towards_zero
          "Number::addition overflow" = .ok res := by
      match hg : g_aln.doRoundUp xn zm_new e_common largeRange.min largeRange.max .towards_zero "Number::addition overflow" with
      | .error e => simp only [hg, reduceCtorEq] at hok'
      | .ok r => exact ⟨r, rfl⟩
    obtain ⟨res, h_rup⟩ := h_rup_exists
    rw [h_rup] at hok'
    have hres_mant_ne : res.mantissa_ ≠ 0 :=
      Number.normalize_mantissa_ne_zero_of_result hok' hresult
    have h_inv := doRoundUp_output_invariants_towards_zero g_aln xn zm_new e_common
      hzm_new_ge_floor hzm_new_le_maxRep "Number::addition overflow" res h_rup hres_mant_ne
    obtain ⟨h_res_min, h_res_max, h_res_exp, h_res_mod⟩ := h_inv
    set res_pos : RoundResult := { negative_ := false, mantissa_ := res.mantissa_, exponent_ := res.exponent_ }
    have hres_pos_mant_ne : res_pos.mantissa_ ≠ 0 := hres_mant_ne
    have h_rup_pos : g_aln.doRoundUp false zm_new e_common largeRange.min largeRange.max .towards_zero "Number::addition overflow" = .ok res_pos :=
      doRoundUp_false_from_ok g_aln xn zm_new e_common .towards_zero "Number::addition overflow" res h_rup
    have h_result_eq_res : result = res.toNumber :=
      Number.normalize_eq_of_invariants_towards_zero h_res_min h_res_max h_res_exp h_res_mod hok'
    have h_result_abs : |result.toRat| = (res_pos.mantissa_.toNat : ℚ) * 10 ^ res_pos.exponent_ := by
      rw [h_result_eq_res, abs_toRat_eq res.toNumber]; rfl
    ...
    refine ⟨zm_new, e_common, f_aln, g_aln, res_pos, hzm_new_ge_floor, hzm_new_le_maxRep,
            hf_aln_nn, hf_aln_lt, h_value_eq, h_rup_pos, h_result_abs,
...
/-- Same-sign branch decomposition of `operator_add` in `.towards_zero` mode. -/
theorem operator_add_algorithmic_facts_same_sign_towards_zero (x y result : Number)
  ...
  have h_abs_add := abs_add_eq_of_same_sign_tz h_same_sign
  ...
  · rw [if_pos h_xe_lt_ye] at hok
    ...
    have hg₀_rep : represents g₀ 0 := represents_initial_guard_eq_tz x.negative_
    ...
    have htruth_eq : |x.toRat + y.toRat|
      ...
      rw [h_abs_add]
      have h_lemma := alignDown_abs_value_tz x g₀ y.exponent_ (le_of_lt h_xe_lt_ye) 0
    ...
    have hok_post : (let zm128 : UInt128 := toUInt128 xm_a + toUInt128 y.mantissa_
      ...
      exact h
    exact operator_add_post_alignment_towards_zero x.negative_ xm_a y.mantissa_ e_common g_aln
  ...
  · push_neg at h_xe_lt_ye
    ...
    · rw [if_neg (not_lt.mpr (le_of_lt h_xe_gt_ye)), if_pos h_xe_gt_ye] at hok
      ...
      have hg₀_rep : represents g₀ 0 := represents_initial_guard_eq_tz y.negative_
      set aln_result : UInt64 × Int × Guard :=
      ...
      have htruth_eq : |x.toRat + y.toRat|
          = ((x.mantissa_.toNat + ym_a.toNat : ℕ) : ℚ) * 10 ^ e_common
            + f_aln * 10 ^ e_common := by
        rw [h_abs_add]
        have h_lemma := alignDown_abs_value_tz y g₀ x.exponent_ (le_of_lt h_xe_gt_ye) 0
        simp only at h_lemma
      ...
      have hok_post : (let zm128 : UInt128 := toUInt128 x.mantissa_ + toUInt128 ym_a
        ...
        rw [he_common_eq]
        exact h
      exact operator_add_post_alignment_towards_zero x.negative_ x.mantissa_ ym_a e_common g_aln
    ...
    · push_neg at h_xe_gt_ye
      ...
      exact operator_add_post_alignment_towards_zero x.negative_ xm_a ym_a e_common g_aln
