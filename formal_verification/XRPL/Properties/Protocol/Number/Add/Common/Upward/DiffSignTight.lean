...
/-- Diff-sign direction for `.upward`: the result never undershoots the truth.
Composes the diff-sign keystone facts (`0 ≤ δ` by the recover digit-exactness)
with the `doNormalize128` direction keystone. -/
