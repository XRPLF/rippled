...
/-! # Algorithmic decomposition for `operator_add` — same-sign branch (downward)

The decomposition produces the same `(zm, ze', f, g, res_pos)` shape as the
`.to_nearest` variant: post the value-equation `|x+y| = (zm + f)·10^ze'` and the
`doRoundUp` invocation. The bound proof for `.downward` adds a sign-restricted
analysis on top of this (it only proves the `Rounds` direction for non-negative
operands; for negative same-sign with cusp, the C++ algorithm does not preserve
the guard's sbit, leading to upward rounding of the magnitude). -/

/-- For same-sign Numbers, `|x.toRat + y.toRat| = |x.toRat| + |y.toRat|`. -/
private lemma abs_add_eq_of_same_sign_dn {x y : Number} (h : x.negative_ = y.negative_) :
...
private lemma represents_initial_guard_eq_dn (xn : Bool) :
...
private lemma alignDown_abs_value_dn
...
/-- `alignDown` preserves the sbit of the input guard. -/
private lemma alignDown_sbit_preserved_dn (m : UInt64) (e : Int) (g : Guard) (target : Int) :
...
/-- Helper: post-alignment processing for the same-sign branch (downward). -/
private theorem operator_add_post_alignment_dn
    (xn : Bool) (xm_a ym_a : UInt64) (e_common : Int) (g_aln : Guard)
    (f_aln : ℚ) (hrep_aln : represents g_aln f_aln)
    (h_g_aln_sbit_pos : xn = false → g_aln.sbit_ = false)
    ...
    (hok : (let zm128 : UInt128 := toUInt128 xm_a + toUInt128 ym_a
            ...
            match p.2.2.doRoundUp xn p.1 p.2.1 largeRange.min largeRange.max
                  .downward "Number::addition overflow" with
            | .error err => Except.error err
            | .ok res => res.toNumber.normalize largeRange.min largeRange.max .downward)
           = .ok result) :
    ∃ (zm : UInt64) (ze' : Int) (f : ℚ) (g : Guard) (res_pos : RoundResult),
      ...
      g.doRoundUp false zm ze' largeRange.min largeRange.max .downward "Number::addition overflow" = .ok res_pos ∧
      |result.toRat| = (res_pos.mantissa_.toNat : ℚ) * 10 ^ res_pos.exponent_ ∧
      res_pos.mantissa_ ≠ 0 ∧
      represents g f ∧
      result.negative_ = xn ∧
      (xn = false → g.sbit_ = false) := by
  ...
  · -- Drop case
    ...
    have h_rup_exists : ∃ res : RoundResult,
        g_new.doRoundUp xn zm_new (e_common + 1) largeRange.min largeRange.max .downward
          "Number::addition overflow" = .ok res := by
      match hg : g_new.doRoundUp xn zm_new (e_common + 1) largeRange.min largeRange.max .downward "Number::addition overflow" with
    ...
    have h_inv := doRoundUp_output_invariants_downward g_new xn zm_new (e_common + 1)
    ...
    have h_rup_pos : g_new.doRoundUp false zm_new (e_common + 1) largeRange.min largeRange.max .downward "Number::addition overflow" = .ok res_pos :=
      doRoundUp_false_from_ok g_new xn zm_new (e_common + 1) .downward "Number::addition overflow" res h_rup
    have h_result_eq_res : result = res.toNumber :=
      Number.normalize_eq_of_invariants_downward h_res_min h_res_max h_res_exp h_res_mod hok'
    ...
    have h_g_new_sbit : xn = false → g_new.sbit_ = false := by
      intro hxn_false
      rw [hg_new_def]
      have h_push_sbit : (g_aln.push d).sbit_ = g_aln.sbit_ := rfl
      rw [h_push_sbit]
      exact h_g_aln_sbit_pos hxn_false
    refine ⟨zm_new, e_common + 1, f_new, g_new, res_pos, hzm_new_ge_floor, hzm_new_le_maxRep,
            ...
            hres_pos_mant_ne, hrep_new, h_res_neg_eq_xn, h_g_new_sbit⟩
  · -- No drop case
    push_neg at h_drop
    ...
    have hok' : (match g_aln.doRoundUp xn zm_new e_common largeRange.min largeRange.max
      ...
      exact h
    have h_rup_exists : ∃ res : RoundResult,
        g_aln.doRoundUp xn zm_new e_common largeRange.min largeRange.max .downward
          "Number::addition overflow" = .ok res := by
      match hg : g_aln.doRoundUp xn zm_new e_common largeRange.min largeRange.max .downward "Number::addition overflow" with
      | .error e => simp only [hg, reduceCtorEq] at hok'
      | .ok r => exact ⟨r, rfl⟩
    obtain ⟨res, h_rup⟩ := h_rup_exists
    rw [h_rup] at hok'
    have hres_mant_ne : res.mantissa_ ≠ 0 :=
      Number.normalize_mantissa_ne_zero_of_result hok' hresult
    have h_inv := doRoundUp_output_invariants_downward g_aln xn zm_new e_common
      hzm_new_ge_floor hzm_new_le_maxRep "Number::addition overflow" res h_rup hres_mant_ne
    obtain ⟨h_res_min, h_res_max, h_res_exp, h_res_mod⟩ := h_inv
    set res_pos : RoundResult := { negative_ := false, mantissa_ := res.mantissa_, exponent_ := res.exponent_ }
    have hres_pos_mant_ne : res_pos.mantissa_ ≠ 0 := hres_mant_ne
    have h_rup_pos : g_aln.doRoundUp false zm_new e_common largeRange.min largeRange.max .downward "Number::addition overflow" = .ok res_pos :=
      doRoundUp_false_from_ok g_aln xn zm_new e_common .downward "Number::addition overflow" res h_rup
    have h_result_eq_res : result = res.toNumber :=
      Number.normalize_eq_of_invariants_downward h_res_min h_res_max h_res_exp h_res_mod hok'
    have h_result_abs : |result.toRat| = (res_pos.mantissa_.toNat : ℚ) * 10 ^ res_pos.exponent_ := by
      rw [h_result_eq_res, abs_toRat_eq res.toNumber]; rfl
    ...
    refine ⟨zm_new, e_common, f_aln, g_aln, res_pos, hzm_new_ge_floor, hzm_new_le_maxRep,
            ...
            hres_pos_mant_ne, hrep_aln, h_res_neg_eq_xn, h_g_aln_sbit_pos⟩

set_option maxHeartbeats 1600000 in
-- large existential with three alignment cases, each unfolding the post-alignment pipeline
/-- Same-sign branch decomposition of `operator_add` in `.downward` mode.

Same shape as the `.to_nearest` decomposition; the bound proof handles the
sign-direction analysis on top. -/
theorem operator_add_algorithmic_facts_same_sign_downward (x y result : Number)
    ...
    ∃ (zm : UInt64) (ze' : Int) (f : ℚ) (g : Guard) (res_pos : RoundResult),
      ...
      (x.negative_ = false → g.sbit_ = false) := by
  ...
  have h_abs_add := abs_add_eq_of_same_sign_dn h_same_sign
  ...
  · rw [if_pos h_xe_lt_ye] at hok
    ...
    have hg₀_rep : represents g₀ 0 := represents_initial_guard_eq_dn x.negative_
    ...
    have htruth_eq : |x.toRat + y.toRat|
      ...
      rw [h_abs_add]
      have h_lemma := alignDown_abs_value_dn x g₀ y.exponent_ (le_of_lt h_xe_lt_ye) 0
    ...
    have hok_post : (let zm128 : UInt128 := toUInt128 xm_a + toUInt128 y.mantissa_
      ...
      exact h
    have h_g_aln_sbit_pos : x.negative_ = false → g_aln.sbit_ = false := by
      intro hxn_false
      rw [hg_aln_def, haln_def, alignDown_sbit_preserved_dn, hg₀_def, if_neg (by rw [hxn_false]; decide)]
      rfl
    have h_result := operator_add_post_alignment_dn x.negative_ xm_a y.mantissa_ e_common g_aln
      f_aln hf_aln_rep' h_g_aln_sbit_pos hxm_a_lt hy_mant_bound hsum_ge (x.toRat + y.toRat) htruth_eq
  ...
  · push_neg at h_xe_lt_ye
    ...
    · rw [if_neg (not_lt.mpr (le_of_lt h_xe_gt_ye)), if_pos h_xe_gt_ye] at hok
      ...
      have hg₀_rep : represents g₀ 0 := represents_initial_guard_eq_dn y.negative_
      set aln_result : UInt64 × Int × Guard :=
      ...
      have htruth_eq : |x.toRat + y.toRat|
          = ((x.mantissa_.toNat + ym_a.toNat : ℕ) : ℚ) * 10 ^ e_common
            + f_aln * 10 ^ e_common := by
        rw [h_abs_add]
        have h_lemma := alignDown_abs_value_dn y g₀ x.exponent_ (le_of_lt h_xe_gt_ye) 0
        simp only at h_lemma
      ...
      have h_g_aln_sbit_pos : x.negative_ = false → g_aln.sbit_ = false := by
        intro hxn_false
        have hyn_false : y.negative_ = false := h_same_sign ▸ hxn_false
        rw [hg_aln_def, haln_def, alignDown_sbit_preserved_dn, hg₀_def, if_neg (by rw [hyn_false]; decide)]
        rfl
      exact operator_add_post_alignment_dn x.negative_ x.mantissa_ ym_a e_common g_aln
        f_aln hf_aln_rep' h_g_aln_sbit_pos hx_mant_bound hym_a_lt hsum_ge (x.toRat + y.toRat) htruth_eq
        result hresult hok_post
    · push_neg at h_xe_gt_ye
      ...
      have h_g_aln_sbit_pos : x.negative_ = false → g_aln.sbit_ = false := by
        intro _; rw [hg_aln_def]; rfl
      exact operator_add_post_alignment_dn x.negative_ xm_a ym_a e_common g_aln
        f_aln hf_aln_rep' h_g_aln_sbit_pos hxm_a_lt hym_a_lt hsum_ge (x.toRat + y.toRat) htruth_eq
        result hresult hok_post
