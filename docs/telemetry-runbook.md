...
| Node Operating State Transitions        | `[DEFAULT OK]` | Transition rate and state timeline from `STATE->` (`NetworkOPsImp::setMode`, warn)                                                                                                         |
...
| _Rotation Phase Duration (p95 by stage)_                    | `freshen.fetch` a few minutes on a populated node; other stages proportional to state-map size                                                                              | `freshen.fetch` p95 in tens of minutes                          | the per-key re-fetch loop is reading the whole tree-node cache back from disk; this is read load, not a freeze (the mutex is released between partitions). Read it next to _Cache Lock Hold Peak_, which is where a freeze would show                                                                                                                                             |
...
| _Cache Lock Hold Peak (us)_                                 | zero on an idle node; sub-millisecond values during a sweep                                                                                                                 | multi-second peak                                               | the `TaggedCache` for either the tree-node cache or the FullBelow cache held its mutex that long across `getKeys()` or `sweep()`. Correlate with the `rotating` log line and the `nodestore.rotate.freshen.fetch` span: a rotation is the usual cause. The freshen copies keys one partition at a time, so a hold above one second here means one partition outgrew a single hold |
| _Rotation Freshen Yield (keys per rotation)_                | `copied` far below `fetched` on both caches                                                                                                                                 | `copied` a large share of `fetched`                             | the copy walk is leaving many cached nodes behind in the archive and the freshen is doing real rescue work; look at `nodes_copied` on the `copy` span before deciding the freshen can be trimmed                                                                                                                                                                                  |
|                                                             |                                                                                                                                                                             | `fetched` in the tens of millions every rotation                | expected today: the freshen re-reads the whole tree-node cache. This is read load, not a freeze; whether it costs sync on its own is what this counter and _Cache Lock Hold Peak_ together settle                                                                                                                                                                                 |
...
>
> 1. Locate the flap time from the `STATE->syncing` warning in Loki
>    (`{service_instance_id="<node>"} |= "STATE->"`). It carries the exact timestamp.
>    `increase(state_changes_total{from="full",to="syncing"}[1m])` only counts flaps: its
>    sample time is up to two minutes late, so never use it to order a flap against
>    another event.
...
> 4. In Loki, `{service_instance_id="<node>"} |~ "held the lock|View of consensus changed"`
>    in a +/- 2 min window: a `TaggedCache <op> held the lock <n>ms` warning (op is
>    `sweep`, `getKeys` or `forEachKeyPartition`) followed within a few seconds by
>    `View of consensus changed` is the freeze and its flap, at second resolution. (Prometheus counters are 60 s-resolution and cannot order the two.)
>    In Tempo, `{ name = "nodestore.rotate.freshen.fetch" }` shows the phase the hold sits in.
