    ...
    private:
        ...
        testInvalidDeposit(FeatureBitset features)
        {
            ...
            testAMM([&](AMM& ammAlice, Env& env) {
                // Invalid flags
                ammAlice.deposit(
                    alice, 1'000'000, std::nullopt, tfWithdrawAll, ter(temINVALID_FLAG));
            ...
            testAMM([&](AMM& amm, Env& env) {
                amm.deposit(
                    DepositArg{.tokens = IOUAmount{1, -12}, .err = ter(tecAMM_INVALID_TOKENS)});
            });
            ...
            testAMM([&](AMM& amm, Env& env) {
                amm.deposit(
                    DepositArg{
                        .asset1In = STAmount{USD, 1, -15}, .err = ter(tecAMM_INVALID_TOKENS)});
            });
        ...
        testBid(FeatureBitset features)
        {
            ...
            testAMM(
                [&](AMM& ammAlice, Env& env) {
                    ...
                    env(ammAlice.bid({.account = bob}));
                    env.close(seconds(
                        (AUCTION_SLOT_TIME_INTERVALS * AUCTION_SLOT_INTERVAL_DURATION) + 1));
                    BEAST_EXPECT(
                        ammAlice.expectAuctionSlot(0, std::nullopt, IOUAmount{127'33875, -5}));
            ...
            testAMM(
                [&](AMM& ammAlice, Env& env) {
                    ...
                    BEAST_EXPECT(
                        ammAlice.expectAuctionSlot(0, 0, IOUAmount{tiny * Number{105, -2}}));
        ...
        testBasicPaymentEngine(FeatureBitset features)
        {
            ...
            testAMM(
                [&](AMM& ammAlice, Env& env) {
                    ...
                    env.close();
                    BEAST_EXPECT(
                        ammAlice.expectBalances(XRP(10'010), USD(10'000), ammAlice.tokens()));
            ...
            testAMM(
                [&](AMM& ammAlice, Env& env) {
                    ...
                    BEAST_EXPECT(expectHolding(
                        env, carol, STAmount{USD, UINT64_C(30'009'09090909091), -11}));
                    BEAST_EXPECT(
                        expectLedgerEntryRoot(env, bob, XRP(30'000) - XRP(10) - txfee(env, 1)));
                },
                {{XRP(10'000), USD(10'010)}},
            ...
            testAMM(
                [&](AMM& ammAlice, Env& env) {
                    ...
                    BEAST_EXPECT(expectOffers(
                        ...
                        2,
                        {{Amounts{
                              XRPAmount(50'487'378), STAmount(EUR, UINT64_C(49'98750312422), -11)},
                          Amounts{
            ...
            testAMM(
                [&](AMM& ammAlice, Env& env) {
                    ...
                    // No transfer fee
                    BEAST_EXPECT(
                        ammAlice.expectBalances(GBP(1'100), EUR(1'000), ammAlice.tokens()));
            ...
            testAMM(
                [&](AMM& ammAlice, Env& env) {
                    ...
                    BEAST_EXPECT(
                        ammAlice.expectBalances(GBP(10'125), EUR(10'000), ammAlice.tokens()));
        ...
        testTradingFee(FeatureBitset features)
        {
            ...
            testAMM(
                [&](AMM& ammAlice, Env& env) {
                    ...
                    BEAST_EXPECT(expectHolding(
                        env, carol, STAmount{USD, UINT64_C(29'994'96220068281), -11}));
                },
                {{USD(1'000), EUR(1'000)}},
                0,
                std::nullopt,
            ...
            testAMM(
                [&](AMM& ammAlice, Env& env) {
                    ...
                    ammAlice.withdrawAll(carol, USD(0));
                    BEAST_EXPECT(expectHolding(
                        env, carol, STAmount{USD, UINT64_C(29'994'97487437186), -11}));
                },
                {{USD(1'000), EUR(1'000)}},
                0,
                std::nullopt,
            ...
            testAMM(
                [&](AMM& ammAlice, Env& env) {
                    ...
                    // to the pool
                    BEAST_EXPECT(expectHolding(
                        env, carol, STAmount{USD, UINT64_C(29'995'02512562814), -11}));
                    BEAST_EXPECT(expectHolding(
                        env, carol, STAmount{EUR, UINT64_C(30'004'97487437186), -11}));
                    BEAST_EXPECT(expectOffers(
                        env,
                        carol,
                        1,
            ...
            // Remaining amount is consumed via AMM liquidity
            {
                ...
                if (!features[fixAMMv1_1])
                {
                    ...
                    BEAST_EXPECT(
                        ammAlice.expectBalances(USD(1'000), EUR(1'005), ammAlice.tokens()));
                }
                else
                {
                    BEAST_EXPECT(
            ...
            // amount is consumed via AMM liquidity.
            {
                ...
                BEAST_EXPECT(expectHolding(env, ed, USD(2'010)));
                BEAST_EXPECT(
                    expectHolding(env, bob, STAmount{EUR, UINT64_C(1'989'993923296712), -12}));
                BEAST_EXPECT(ammAlice.expectBalances(
                    USD(1'004),
        ...
        testAdjustedTokens(FeatureBitset features)
        {
            ...
            testAMM(
                [&](AMM& ammAlice, Env& env) {
                    ...
                    {
                        BEAST_EXPECT(expectHolding(
                            env, alice, STAmount{USD, UINT64_C(30'000'0000000003), -10}));
                    }
                    else
                    {
            ...
            testAMM(
                [&](AMM& ammAlice, Env& env) {
                    ...
                    else
                    {
                        ...
                        BEAST_EXPECT(
                            accountBalance(env, paul) == (xrpBalance + drops(3)).getText());
                        BEAST_EXPECT(
                            accountBalance(env, natalie) == (xrpBalance + drops(5)).getText());
                        BEAST_EXPECT(
                            accountBalance(env, alice) ==
        ...
        testFixDefaultInnerObj()
        {
            ...
            test(
                all - fixInnerObjTemplate, tesSUCCESS, tesSUCCESS, tesSUCCESS, tesSUCCESS, 0, true);
        ...
        testAMMClawback(FeatureBitset features)
        {
            ...
            else
            {
                ...
                // is confusing.
                auto const error = features[featureSingleAssetVault] ? ter{tecPSEUDO_ACCOUNT}
                                                                     : ter{tecAMM_ACCOUNT};
        ...
        testAMMDepositWithFrozenAssets(FeatureBitset features)
        {
            ...
            // then we should get tecFROZEN error.
            {
                ...
                testAMMDeposit(env, [&](AMM& amm) {
                    amm.deposit(
                        alice, USD(100), XRP(100), std::nullopt, tfTwoAsset, ter(tecFROZEN));
                });
            }
        ...
        testDepositAndWithdrawRounding(FeatureBitset features)
        {
            ...
            test(
                [&](AMM& amm, Env& env) {
                    auto const err =
                        env.enabled(fixAMMv1_3) ? ter(tesSUCCESS) : ter(tecUNFUNDED_AMM);
                    ...
                    [&](AMM& ammAlice, Env& env) {
                        ...
                        ammAlice.deposit(
                            DepositArg{
                                .account = bob, .asset1In = depositEuro, .asset2In = depositGBP});
                        invariant(
                            ammAlice, env, "dep3", exponent != -3 && !env.enabled(fixAMMv1_3));
                    },
