...
private:
    ...
    testOfferFeesConsumeFunds(FeatureBitset features)
    {
        ...
        env.fund(starting_xrp, gw_, alice_);
    ...
    testGatewayCrossCurrency(FeatureBitset features)
    {
        ...
        env.fund(starting_xrp, gw_, alice_, bob_);
    ...
    testDirectToDirectPath(FeatureBitset features)
    {
        ...
        auto const bob_ = Account("bob");
        auto const cam = Account("cam");
        auto const carol_ = Account("carol");

        auto const fee = env.current()->fees().base;
        env.fund(XRP(1'000), carol_);
        env.fund(reserve(env, 4) + (fee * 5), ann, bob_, cam);
        env.close();

        MPTTester const A_BUX(
            {.env = env, .issuer = ann, .holders = {bob_, cam, carol_}, .flags = kMPT_DEX_FLAGS});

        MPTTester const B_BUX(
            {.env = env, .issuer = bob_, .holders = {ann, cam, carol_}, .flags = kMPT_DEX_FLAGS});

        env(pay(ann, cam, A_BUX(350'000'000'000'000)));
        env(pay(bob_, cam, B_BUX(350'000'000'000'000)));
        env(pay(bob_, carol_, B_BUX(4'000'000'000'000'000)));
        env(pay(ann, carol_, A_BUX(4'000'000'000'000'000)));

        AMM const ammCarol(env, carol_, A_BUX(3'000'000'000'000'000), B_BUX(3'300'000'000'000'000));
    ...
    testBookStep(FeatureBitset features)
    {
...

        {
            ...
            AMM const ammBobBTC_XRP(env, bob_, BTC(100'000), XRP(150));
            AMM const ammBobXRP_ETH(env, bob_, XRP(100), ETH(150'000));
...

        {
            ...
            AMM const ammBobBTC_ETH(env, bob_, BTC(50'000'000), ETH(50'000'000));
    ...
    testXRPPathLoop()
    {
        ...
        // Payment path starting with XRP
        {
            auto test = [&](auto&& issue1, auto&& issue2) {
                ...
                AMM const ammAliceXRP_BTC(env, alice_, XRP(100), BTC(101'000));
                AMM const ammAliceXRP_ETH(env, alice_, XRP(100), ETH(101'000));
        ...
        // Payment path ending with XRP
        {
            auto test = [&](auto&& issue1, auto&& issue2) {
                ...
                AMM const ammAliceXRP_BTC(env, alice_, XRP(100), BTC(100'000));
                AMM const ammAliceXRP_ETH(env, alice_, XRP(100), ETH(100'000));
                // ETH -> //XRP -> //BTC ->XRP
        ...
        // on an endpoint
        {
            auto test = [&](auto&& issue1, auto&& issue2, auto&& issue3) {
                ...
                AMM const ammAliceXRP_BTC(env, alice_, XRP(100), BTC(100'000));
                AMM const ammAliceXRP_ETH(env, alice_, XRP(100), ETH(100'000));
                AMM const ammAliceXRP_JPY(env, alice_, XRP(100), JPY(100'000));
    ...
    test_convert_all_of_an_asset(FeatureBitset features)
    {
...

        {
            ...
            env(pay(alice_, bob_, BTC(10'000)), deliver_min(BTC(10'000)), Ter(temBAD_AMOUNT));
    ...
    testIndividualLock(FeatureBitset features)
    {
        ...
        env(pay(G1, bob_, BTC(10)));
        env(pay(G1, alice_, BTC(205)));
        env.close();

        AMM const ammAlice(env, alice_, XRP(500), BTC(105));

        env.Require(Balance(bob_, BTC(10)));
        env.Require(Balance(alice_, BTC(100)));

        // Account with MPT unlocked (proving operations normally work)
        // can make Payment
        env(pay(alice_, bob_, BTC(1)));

        // can receive Payment
        env(pay(bob_, alice_, BTC(1)));
        env.close();

        // Lock MPT for bob
        BTC.set({.holder = bob_, .flags = tfMPTLock});

        {
            // different from IOU. The offer is created but not crossed.
            env(offer(bob_, BTC(5), XRP(25)));
            env.close();
            BEAST_EXPECT(expectOffers(env, bob_, 1, {{{BTC(5), XRP(25)}}}));
            BEAST_EXPECT(ammAlice.expectBalances(XRP(500), BTC(105), ammAlice.tokens()));
        }

        {
            // can not sell assets
            env(offer(bob_, XRP(1), BTC(5)), Ter(tecUNFUNDED_OFFER));

            // different from IOU
            // can not receive Payment when locked
            env(pay(alice_, bob_, BTC(1)), Ter(tecPATH_DRY));

            // can not make Payment when locked
            env(pay(bob_, alice_, BTC(1)), Ter(tecPATH_DRY));
        ...
        {
            // Unlock
            BTC.set({.holder = bob_, .flags = tfMPTUnlock});
            env(offer(bob_, XRP(1), BTC(5)));
            env(pay(bob_, alice_, BTC(1)));
            env(pay(alice_, bob_, BTC(1)));
            env.close();
        }
    ...
    testTxMultisign(FeatureBitset features)
    {
        ...
        Account const alice_{"alice", KeyType::secp256k1};
        Account const becky{"becky", KeyType::ed25519};
        Account const zelda{"zelda", KeyType::secp256k1};
        fund(env, gw_, {alice_, becky, zelda}, XRP(20'000));

        MPTTester const BTC(
            {.env = env,
             .issuer = gw_,
             .holders = {alice_, becky, zelda},
             .pay = 20'000'000'000,
             .flags = kMPT_DEX_FLAGS});
        ...
        env.Require(Owners(alice_, signerListOwners + 0));
    ...
    testToStrand(FeatureBitset features)
    {
        ...
        // cannot have more than one offer with the same output issue
        {
            auto test = [&](auto&& issue1, auto&& issue2) {
                ...
                env.close();
                AMM const bobXRP_BTC(env, bob_, XRP(1'000), BTC(1'000));
                AMM const bobBTC_ETH(env, bob_, BTC(1'000), ETH(1'000));
    ...
    testRIPD1373(FeatureBitset features)
    {
...

        {
            ...
            AMM const ammXRP_BTC(env, bob_, XRP(100), BTC(100'000));
            env(offer(gw_, XRP(100), BTC(100'000)), txflags(tfPassive));

            AMM const ammBTC_ETH(env, bob_, BTC(100'000), ETH(100'000));
            env(offer(gw_, BTC(100'000), ETH(100'000)), txflags(tfPassive));

            TestPath const p = [&] {
                TestPath result;
                result.push_back(allPathElements(gw_, MPT(BTC)));
    ...
    testLoop(FeatureBitset features)
    {
...

        {
            auto test = [&](auto&& issue1, auto&& issue2, auto&& issue3) {
                ...
                AMM const ammBobXRP_BTC(env, bob_, XRP(100), BTC(100));
                AMM const ammBobBTC_ETH(env, bob_, BTC(100), ETH(100));
                AMM const ammBobETH_CNY(env, bob_, ETH(100), CNY(100));
