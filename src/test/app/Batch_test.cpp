...
class Batch_test : public beast::unit_test::Suite
{
    ...
    testBatchTxQueue(FeatureBitset features)
    {
        ...
        // tracked in the TxQ.
        {
            ...
            // The first close applies the outer Batch (and Carol's regular
            // tx); the second applies its inners (Batch inners only apply once
            // the outer is in a closed ledger).
            env.close();
            env.close();
