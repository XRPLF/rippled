...
public:
        ...
        jtx::PrettyAmount const& expectBalance)
    {
        ...
        auto const sleTrust = env.le(keylet::rippleState(account.id(), issue));
        ...
        for (auto const& t : tests)
        {
            ...
            if (t.preTrust == preTrustType::noPreTrust)
            {
                ...
                else
                {
                    ...
                    auto const sleTrust = env.le(keylet::rippleState(acct, USD));
                    BEAST_EXPECT(!sleTrust);
                }
            }
    ...
    testDirectCross(FeatureBitset features)
    {
        ...
        BEAST_EXPECT(!env.le(keylet::rippleState(alice.id(), EUR)));
        BEAST_EXPECT(!env.le(keylet::rippleState(bob.id(), USD)));
    ...
    testDeletedOfferIssuer(FeatureBitset features)
    {
        ...
        auto trustLineExists = [](jtx::Env const& env,
            ...
            return bool(env.le(keylet::rippleState(src, dst, cur)));
        };
