...
struct EscrowToken_test : public beast::unit_test::Suite
{
    ...
    testIOUCancelDoApply(FeatureBitset features)
    {
...

        {
            ...
            {
                BEAST_EXPECT(!env.le(keylet::escrow(alice.id(), SeqProxy::sequence(seq))));
                BEAST_EXPECT(env.current()->exists(trustLineKey));
    ...
    testIOUMetaAndOwnership(FeatureBitset features)
    {
        ...
        auto const usd = gw["USD"];
        {
            ...
            env.close(5s);
            auto const aa = env.le(keylet::escrow(alice.id(), SeqProxy::sequence(aseq)));
            BEAST_EXPECT(aa);
            {
            ...
            BEAST_EXPECT(
                (*env.meta())[sfTransactionResult] == static_cast<std::uint8_t>(tesSUCCESS));
            env.close(5s);
            auto const bb = env.le(keylet::escrow(bob.id(), SeqProxy::sequence(bseq)));
            BEAST_EXPECT(bb);
            ...
            {
                BEAST_EXPECT(!env.le(keylet::escrow(alice.id(), SeqProxy::sequence(aseq))));
                BEAST_EXPECT(
                    (*env.meta())[sfTransactionResult] == static_cast<std::uint8_t>(tesSUCCESS));
            ...
            {
                BEAST_EXPECT(!env.le(keylet::escrow(bob.id(), SeqProxy::sequence(bseq))));
                BEAST_EXPECT(
                    (*env.meta())[sfTransactionResult] == static_cast<std::uint8_t>(tesSUCCESS));
        ...
        }
        {
            ...
            auto const ab = env.le(keylet::escrow(alice.id(), SeqProxy::sequence(aseq)));
            BEAST_EXPECT(ab);

            auto const bc = env.le(keylet::escrow(bob.id(), SeqProxy::sequence(bseq)));
            BEAST_EXPECT(bc);
...

        {
            ...
            auto const ag = env.le(keylet::escrow(alice.id(), SeqProxy::sequence(aseq)));
            BEAST_EXPECT(ag);
    ...
    testMPTFinishPreclaim(FeatureBitset features)
    {
        ...
        // tecOBJECT_NOT_FOUND: MPT issuance does not exist
        {
            ...
            env.app().getOpenLedger().modify([&](OpenView& view, beast::Journal j) {
                ...
                auto sleNew =
                    std::make_shared<SLE>(keylet::escrow(alice, SeqProxy::sequence(seq1)));
