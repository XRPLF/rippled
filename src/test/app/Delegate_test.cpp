...
class Delegate_test : public beast::unit_test::Suite
{
    ...
    testReserve()
    {
        ...
        // reserve requirement not met
        {
            ...
            env.fund(baseAccountReserve(*env.current(), {.ownerCountDelta = 0}) + txFee, alice);
        ...
        // reserve recovered after deleting delegation object
        {
            ...
            env.fund(
                baseAccountReserve(*env.current(), {.ownerCountDelta = 1}) + (txFee * 4), alice);
        ...
        // test reserve when sending transaction on behalf of other account
        {
            ...
            env.fund(drops(baseAccountReserve(*env.current(), {.ownerCountDelta = 1})), alice);
            env.fund(drops(baseAccountReserve(*env.current(), {.ownerCountDelta = 2})), bob);
            env.close();
    ...
    testFee()
    {
        ...
        // The delegated account has enough balance to pay and delegator has enough reserve
        {
            ...
            auto setup = [&](Env& env) {
                ...
                auto const reserve = baseAccountReserve(*env.current(), {.ownerCountDelta = 1});
        ...
        // Delegated account can pay the fee even if it dips below reserve.
        {
            ...
            auto const baseFee = env.current()->fees().base;
            auto const baseReserve = baseAccountReserve(*env.current(), {.ownerCountDelta = 0});

            env.fund(
                baseAccountReserve(*env.current(), {.ownerCountDelta = 1}) + baseFee + XRP(1),
                alice);
