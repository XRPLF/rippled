...
class LPTokenTransfer_test : public jtx::AMMTest
{
    ...
    testCheck(FeatureBitset features)
    {
        ...
        // token
        uint256 const carolChkId{keylet::check(carol_, SeqProxy::sequence(env.seq(carol_))).key};
        ...
        uint256 const bobChkId{keylet::check(bob_, SeqProxy::sequence(env.seq(bob_))).key};
    ...
    testNFTOffers(FeatureBitset features)
    {
        ...
        uint256 const sellOfferIndex =
            keylet::nftokenOffer(bob_, SeqProxy::sequence(env.seq(bob_))).key;
