    ...
    private:
        ...
        testEnforceNoRipple(FeatureBitset features)
        {
...

            {
                ...
                BEAST_EXPECT(
                    expectLedgerEntryRoot(env, alice, XRP(20'000) - XRP(50) - txfee(env, 1)));
        ...
        testFillModes(FeatureBitset features)
        {
            ...
            testAMM(
                [&](AMM& ammAlice, Env& env) {
                    ...
                    BEAST_EXPECT(
                        ammAlice.expectBalances(XRP(10'000), USD(10'100), ammAlice.tokens()));
                    BEAST_EXPECT(
        ...
        testSellFlagBasic(FeatureBitset features)
        {
            ...
            testAMM(
                [&](AMM& ammAlice, Env& env) {
                    ...
                    env.close();
                    BEAST_EXPECT(
                        ammAlice.expectBalances(XRP(10'000), USD(9'999), ammAlice.tokens()));
                    BEAST_EXPECT(expectOffers(env, carol, 0));
        ...
        testGatewayCrossCurrency(FeatureBitset features)
        {
            ...
            if (!features[fixAMMv1_1])
            {
                ...
                BEAST_EXPECT(
                    expectHolding(env, bob, STAmount{XTS, UINT64_C(98'989898989899), -12}));
            }
            else
            {
                BEAST_EXPECT(ammAlice.expectBalances(
                ...
                BEAST_EXPECT(
                    expectHolding(env, bob, STAmount{XTS, UINT64_C(98'9898989898989), -13}));
            }
        ...
        path_find_consume_all()
        {
            ...
            std::tie(st, sa, da) = find_paths(
                env, alice, bob, bob["AUD"](-1), std::optional<STAmount>(XRP(100'000'000)));
            BEAST_EXPECT(st.empty());
            std::tie(st, sa, da) = find_paths(
                env, alice, bob, bob["USD"](-1), std::optional<STAmount>(XRP(100'000'000)));
        ...
        testBookStep(FeatureBitset features)
        {
            ...
            }
            {
                ...
                BEAST_EXPECT(expectHolding(env, carol, USD(200)));
                BEAST_EXPECT(
                    ammBobBTC_XRP.expectBalances(BTC(150), XRP(100), ammBobBTC_XRP.tokens()));
                BEAST_EXPECT(
                    ammBobXRP_USD.expectBalances(XRP(150), USD(100), ammBobXRP_USD.tokens()));
            }
            {
                ...
                BEAST_EXPECT(
                    expectLedgerEntryRoot(env, bob, XRP(10'000) - XRP(100) - ammCrtFee(env)));
                BEAST_EXPECT(expectHolding(env, bob, USD(0)));
                BEAST_EXPECT(expectHolding(env, carol, USD(200)));
            ...
            }
            {
                ...
                BEAST_EXPECT(
                    expectLedgerEntryRoot(env, bob, XRP(10'000) - XRP(150) - ammCrtFee(env)));
                BEAST_EXPECT(expectHolding(env, bob, USD(0)));
        ...
        testTransferRateNoOwnerFee(FeatureBitset features)
        {
            ...
            }
            {
                ...
                fund(
                    env, gw, {alice, bob, carol}, XRP(1'000), {USD(1'400), EUR(1'400), GBP(1'400)});
                env(rate(gw, 1.25));
                env.close();
        ...
        test_convert_all_of_an_asset(FeatureBitset features)
        {
...

            {
                ...
                env(pay(alice, bob, USD(10)),
                    ...
                    ter(tecPATH_PARTIAL));
                env.require(
                    balance(alice, drops(10'000'000'000 - env.current()->fees().base.drops())));
        ...
        testPayment(FeatureBitset features)
        {
            ...
            env.close();
            BEAST_EXPECT(
                ammAlice.expectBalances(XRPAmount(107'692'308), USD(130), ammAlice.tokens()));
        ...
        testTxMultisign(FeatureBitset features)
        {
            ...
            // 4000 tokens burnt
            BEAST_EXPECT(
                ammAlice.expectBalances(XRP(10'000), USD(10'000), IOUAmount{9'996'000, 0}));
        }
