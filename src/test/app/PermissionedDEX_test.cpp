...
class PermissionedDEX_test : public beast::unit_test::Suite
{
    ...
    testHybridInvalidOffer(FeatureBitset features)
    {
        ...
        if (features[fixCleanup3_2_0])
        {
            ...
            auto const carolBalBefore = env.balance(carol_, USD);
            env(pay(alice_, carol_, USD(5)), Path(~USD), Sendmax(XRP(5)));
            env.close();
            BEAST_EXPECT(env.balance(carol_, USD) - carolBalBefore == USD(5));
            BEAST_EXPECT(checkOffer(env, bob_, hybridOfferSeq, XRP(45), USD(45), lsfHybrid, true));
            ...
            BEAST_EXPECT(checkOffer(env, bob_, hybridOfferSeq, XRP(40), USD(40), lsfHybrid, true));
            BEAST_EXPECT(checkOffer(env, bob_, regularOfferSeq, XRP(10), USD(10)));
            BEAST_EXPECT(checkDirectorySize(env, openDir, 2));
        }
        else
    ...
    testHybridOpenBookAfterCredentialExpiry(FeatureBitset features)
    {
        ...
        auto carolBalance = env.balance(carol_, USD);
        env(pay(alice_, carol_, USD(5)), Path(~USD), Sendmax(XRP(5)));
        env.close();
        BEAST_EXPECT(env.balance(carol_, USD) - carolBalance == USD(5));
        ...
        carolBalance = env.balance(carol_, USD);
        env(pay(alice_, carol_, USD(2)), Path(~USD), Sendmax(XRP(2)));
        env.close();

        // Carol received USD; the offer was crossed, not evicted.
        BEAST_EXPECT(env.balance(carol_, USD) - carolBalance == USD(2));
        ...
        carolBalance = env.balance(carol_, USD);
        env(pay(alice_, carol_, USD(3)), Path(~USD), Sendmax(XRP(3)));
        env.close();
        BEAST_EXPECT(env.balance(carol_, USD) - carolBalance == USD(3));
        BEAST_EXPECT(!offerExists(env, devin, hybridOfferSeq));
    }
