...
/* "fund" creates becky's acct   */ { 5, jss::Payment,              {jss::AccountRoot}, {},                 {jss::AccountRoot}}
    ...
    testSponsorship()
    {
        ...
        // fee sponsorship
        env(noop(alice), sponsor::as(sponsor, tfSponsorFee), sig(sfSponsorSignature, sponsor));
        env.close();
        ...
        // use a ticket
        env(noop(alice), ticket::use(seq + 1), sponsor::as(sponsor, tfSponsorFee), sig(sfSponsorSignature, sponsor));
        env.close();
        checkTx(alice, jss::AccountSet);
        checkTx(sponsor, jss::AccountSet);
