...
class Version_test : public beast::unit_test::suite
{
    ...
    testWrongVersionNumber()
    {
        ...
        re = env.rpc(
            ...
            "{\"api_version\": " +
                std::to_string(
                    std::max(
                        std::integral_constant<unsigned int, 2>::value,
                        std::integral_constant<unsigned int, 3>::value) +
                    1) +
                "}");
        BEAST_EXPECT(badVersion(re));
    ...
    testGetAPIVersionNumber()
    {
        ...
        j_object[jss::api_version] = std::integral_constant<unsigned int, 1>::value;
        BEAST_EXPECT(RPC::getAPIVersionNumber(j_object, false) == versionIfUnspecified);

        j_object[jss::api_version] = std::integral_constant<unsigned int, 1>::value;
        BEAST_EXPECT(RPC::getAPIVersionNumber(j_object, false) == RPC::apiMinimumSupportedVersion);
        j_object[jss::api_version] = std::integral_constant<unsigned int, 2>::value;
        ...
        BEAST_EXPECT(RPC::getAPIVersionNumber(j_object, false) == RPC::apiInvalidVersion);
        j_object[jss::api_version] = std::integral_constant<unsigned int, 3>::value;
        ...
        j_object[jss::api_version] = std::integral_constant<unsigned int, 0>::value;
        BEAST_EXPECT(RPC::getAPIVersionNumber(j_object, false) == RPC::apiInvalidVersion);
