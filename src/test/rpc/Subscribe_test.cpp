...
public:
    ...
    testTransactionsAPIv1()
    {
...

        {
            ...
            stream = json::ValueType::ObjectValue;
    ...
    testTransactionsAPIv2()
    {
        ...
        auto wsc = makeWSClient(env.app().config());
        json::Value stream{json::ValueType::ObjectValue};
    ...
    testSubByUrl()
    {
        ...
        jv[jss::streams] = json::ValueType::ArrayValue;
    ...
    testSubErrors(bool subscribe)
    {
        ...
        for (auto const& f : {jss::accounts_proposed, jss::accounts})
        {
            ...
            {
                json::Value jv;
                jv[f] = json::ValueType::ArrayValue;
                auto const jr = wsc->invoke(method, jv)[jss::result];
        ...
        {
            json::Value jv;
            jv[jss::books] = json::ValueType::ArrayValue;
            jv[jss::books][0u] = 1;
            auto const jr = wsc->invoke(method, jv)[jss::result];
            BEAST_EXPECT(jr[jss::error] == "invalidParams");
            BEAST_EXPECT(jr[jss::error_message] == "Invalid parameters.");
...

        {
            json::Value jv;
            jv[jss::books] = json::ValueType::ArrayValue;
            jv[jss::books][0u] = json::ValueType::ObjectValue;
            jv[jss::books][0u][jss::taker_gets] = json::ValueType::ObjectValue;
            jv[jss::books][0u][jss::taker_pays] = json::ValueType::ObjectValue;
            auto const jr = wsc->invoke(method, jv)[jss::result];
    ...
    testHistoryTxStream()
    {
...

        {
            ...
            json::Value request;
            request[jss::account_history_tx_stream] = json::ValueType::ObjectValue;
    ...
    testSubBookChanges()
    {
        ...
        json::Value streams;
        streams[jss::streams] = json::ValueType::ArrayValue;
