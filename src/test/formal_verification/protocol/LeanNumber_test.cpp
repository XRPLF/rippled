...
public:
    ...
    // operator/= Upward at the kMaxRep cusp on the fix-enabled (Large) scale.
    ...
    // operator* Downward at the kMaxRep cusp on the fix-enabled (Large) scale.
    ...
    {
        auto const lean = LeanXRPResult::fromLean(lean_number_to_rep(
