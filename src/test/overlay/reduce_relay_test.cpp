...
public:
    ...
    virtual void deletePeer(Peer::IdT, UnsquelchCB) = 0;
};
...
class Validator
{
    using Links = std::unordered_map<Peer::IdT, LinkSPtr>;
...
public:
    ...
    void
    forLinks(std::vector<Peer::IdT> peers, LinkIterCB f)
    {
        for (auto id : peers)
        {
     ...
     */
    void
    send(std::vector<Peer::IdT> peers, SquelchCB f)
    {
...
{
    using Peers = std::unordered_map<Peer::IdT, PeerSPtr>;
...
public:
    ...
    void
    deletePeer(IdT id, UnsquelchCB f) override
    {
        unsquelch_ = f;
    ...
    void
    deletePeer(Peer::IdT id, bool useCache = true)
    {
    ...
    bool
    isSelected(PublicKey const& validator, Peer::IdT peer)
    {
...
private:
    void
    squelch(PublicKey const& validator, Peer::IdT id, std::uint32_t squelchDuration) const override
    {
...
public:
    ...
    void
    enableLink(std::uint16_t validatorId, Peer::IdT peer, bool enable)
    {
    ...
    void
    onDisconnectPeer(Peer::IdT peer)
    {
    ...
    random(bool log)
    {
        ...
        network_.propagate([&](Link& link, MessageSPtr m) {
            ...
            if (events[EventType::PeerDisconnected].state == State::On)
            {
                ...
                bool const allCounting = network_.allCounting(event.peer);
                network_.overlay().deletePeer(
                    event.peer, [&](PublicKey const& v, PeerWPtr const& peerPtr) {
                        if (event.isSelected)
                ...
                // (allCounting is true)
                bool const handled = (!event.isSelected && !event.handled) ||
                    (event.isSelected && (event.handled || allCounting));
                BEAST_EXPECT(handled);
                event.state = State::Off;
                event.isSelected = false;
                event.handledCnt += handled;
                event.handled = false;
                network_.onDisconnectPeer(event.peer);
            }
                 ...
                 (now - event.time > (reduce_relay::kIdled + seconds(2)))))
            {
                ...
                event.handledCnt += handled;
                event.handled = false;
                network_.enableLink(event.validator, event.peer, true);
            }
        });
...
vp_base_squelch_max_selected_peers=2
    ...
    struct Handler : public reduce_relay::SquelchHandler
    {
        Handler() = default;
        void
        squelch(PublicKey const&, Peer::IdT, std::uint32_t duration) const override
        {
            maxDuration = std::max<uint32_t>(duration, maxDuration);
        }
        void
        unsquelch(PublicKey const&, Peer::IdT) const override
        {
        }
