...
protected:
    ...
    std::vector<ValidatorToken> const validators = makeValidators(3);
    std::uint32_t const now = 1000;
    UnsignedList const list = parseUnsignedList(unsignedListText(validators, 7, now + 100));
    ...
    {
        auto const report = verifyList(doc, roster, key, at.value_or(now));
    ...
    {
        auto const report = verifyList(doc, std::nullopt, std::nullopt, now);
...
{
    {
        auto const l = parseUnsignedList(unsignedListText(validators, 5, 1000, 500));
        EXPECT_EQ(l.sequence, 5u);
        EXPECT_EQ(l.expiration, 1000u);
        ASSERT_TRUE(l.effective);
        EXPECT_EQ(*l.effective, 500u);
        EXPECT_EQ(l.validators.size(), 3u);
        EXPECT_EQ(
            l.validators[0], deserializeManifest(base64Decode(validators[0].manifest))->masterKey);
    }
    {
        auto const l = parseUnsignedList(unsignedListText(validators, 5, 1000));
    ...
    }
    {
        auto const key =
            strHex(deserializeManifest(base64Decode(validators[0].manifest))->masterKey);
        auto const other =
            strHex(deserializeManifest(base64Decode(validators[1].manifest))->masterKey);
        ...
        EXPECT_EQ(
            errorOfParse(entry(other, "\"" + validators[0].manifest + "\"")),
...
TEST_F(ListSigningTest, files)
{
    ...
    EXPECT_EQ(token.manifest, publisher.token.manifest);
    EXPECT_TRUE(sameSecret(token.validationSecret, publisher.token.validationSecret));
    ...
    EXPECT_EQ(manifest.masterKey, publisher.manifest.masterKey);
    EXPECT_EQ(manifest.signingKey, publisher.manifest.signingKey);
    ...
    writeFile(listFile, unsignedListText(validators, 3, 5000));
...
TEST_F(ListSigningTest, sign_and_verify_version_1)
{
    ...
    EXPECT_EQ(v1[jss::public_key].asString(), strHex(publisher.manifest.masterKey));
    EXPECT_EQ(v1[jss::manifest].asString(), publisher.token.manifest);
    EXPECT_EQ(base64Decode(v1[jss::blob].asString()), list.canonical);
    EXPECT_EQ(v1[jss::signature].asString(), signature);

    auto const report = verifyList(v1, list, publisher.manifest.masterKey, now);
    EXPECT_TRUE(report["ok"].asBool()) << to_string(report);
    ...
    EXPECT_EQ(report["signing_key"].asString(), strHex(*publisher.manifest.signingKey));
...
TEST_F(ListSigningTest, sign_and_verify_version_2)
{
    ...
    EXPECT_EQ(v2b[jss::blobs_v2][0u][jss::signature].asString(), signature);
    {
        auto const report = verifyList(v2b, std::nullopt, std::nullopt, now);
        EXPECT_TRUE(report["ok"].asBool()) << to_string(report);
        EXPECT_EQ(report["blobs"][1u][jss::effective].asUInt(), now + 200);
    }
    ...
    auto const resign = [&](std::string const& bytes) {
        return strHex(sign(*manifest2.signingKey, token2.validationSecret, makeSlice(bytes)));
    };
    auto const third = parseUnsignedList(unsignedListText(validators, 9, now + 400, now + 350));
    EXPECT_EQ(
        errorOf([&] {
            makeSignedList(
    ...
    EXPECT_NE(v2c[jss::blobs_v2][0u][jss::signature].asString(), signature);
    EXPECT_EQ(
    ...
    {
        auto const report = verifyList(v2c, std::nullopt, std::nullopt, now);
        EXPECT_TRUE(report["ok"].asBool()) << to_string(report);
        EXPECT_EQ(report["manifest_sequence"].asUInt(), 2u);
        EXPECT_EQ(report["signing_key"].asString(), strHex(*manifest2.signingKey));
    }
...
TEST_F(ListSigningTest, verify_rejects)
{
    ...
    expectError(good, "blob 0: expired", std::nullopt, std::nullopt, now + 100);
    {
        Publisher const other;
        auto wrong = good;
    ...
    {
        auto const roster = parseUnsignedList(unsignedListText(makeValidators(2), 1, now + 100));
    ...
    }
    {
        ...
        expectError(bad, "\"public_key\" and \"manifest\" must be strings");
    }
    {
        auto bad = good;
    ...
    }
    {
        ...
        expectError(bad, "a version 1 list needs \"blob\" and \"signature\" and no \"blobs_v2\"");
    }
    ...
    }
    {
        ...
        SigningKeys revoked = publisher.keys;
    ...
    }
    {
        ...
        auto const token2 = rotated.createToken(KeyType::Ed25519);
        auto const manifest2 = *deserializeManifest(base64Decode(token2.manifest));
        auto const later = parseUnsignedList(unsignedListText(validators, 8, now + 300));
        json::Value entry(json::ValueType::Object);
        entry[jss::blob] = base64Encode(later.canonical);
        entry[jss::signature] = signList(later, *manifest2.signingKey, token2.validationSecret);
        entry[jss::manifest] = token2.manifest;
        auto newer = v2;
        newer[jss::blobs_v2].append(entry);
        auto const report = verifyList(newer, std::nullopt, std::nullopt, now);
        EXPECT_TRUE(report["ok"].asBool()) << to_string(report);
        EXPECT_EQ(report["manifest_sequence"].asUInt(), 2u);
