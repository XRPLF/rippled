...
protected:
    TempDir dir;
    std::filesystem::path keyFile{dir.file("validator_keys.json")};

    void
    writeKeyFile(json::Value const& jv)
    {
        writeFile(keyFile, jv.toStyledString());
    }
    ...
    {
        writeKeyFile(jv);
        return errorOf([&] { SigningKeys::make_SigningKeys(keyFile); });
    }
    ...
    keyFileJson()
    {
        json::Reader reader;
        json::Value jv;
        reader.parse(readFile(keyFile), jv);
        return jv;
    }
...
manifestOf(std::string const& base64, SigningKeys const& keys)
{
    ...
    if (!m)
    {
        ADD_FAILURE() << "not a manifest: " << base64;
        throw std::runtime_error("not a manifest");
    }
...
TEST_F(SigningKeysTest, key_file_round_trip)
{
    for (auto const keyType : kKeyTypes)
    {
        ...
        EXPECT_TRUE(std::filesystem::exists(keyFile));
        EXPECT_FALSE(std::filesystem::exists(keyFile.string() + ".tmp"));
        auto const perms = std::filesystem::status(keyFile).permissions();
        EXPECT_EQ(
            perms & (std::filesystem::perms::group_all | std::filesystem::perms::others_all),
            std::filesystem::perms::none);
        EXPECT_TRUE(keys == SigningKeys::make_SigningKeys(keyFile));
    }
    ...
    other.writeToFile(keyFile);
    EXPECT_TRUE(other == SigningKeys::make_SigningKeys(keyFile));
...
TEST_F(SigningKeysTest, write_to_file_errors)
{
    ...
    auto const nested = dir.file("a/b/c/validator_keys.json");
    keys.writeToFile(nested);
    EXPECT_TRUE(keys == SigningKeys::make_SigningKeys(nested));

    // The parent path is a file
    auto const blocked = std::filesystem::path(keyFile.string() + "/keys.json");
    keys.writeToFile(keyFile);
    EXPECT_EQ(
    ...
    auto const directory = std::filesystem::path(dir.file("dir"));
    ...
    auto const sealed = std::filesystem::path(dir.file("sealed"));
...
{
    EXPECT_EQ(
        errorOf([&] { SigningKeys::make_SigningKeys(keyFile); }),
...
TEST_F(SigningKeysTest, external_key_file_fields)
{
    ...
    jv["token_sequence"] = 0;
    jv["revoked"] = false;
    EXPECT_EQ(loadError(jv), "Key file '" + keyFile.string() + "' is missing \"public_key\" field");
    ...
    auto const keys = SigningKeys::make_SigningKeys(keyFile);
...
TEST_F(SigningKeysTest, create_token)
{
    for (auto const keyType : kKeyTypes)
    {
        ...
        for (auto const tokenKeyType : kKeyTypes)
        {
            ...
            EXPECT_EQ(*m.signingKey, derivePublicKey(tokenKeyType, token.validationSecret));
...
TEST_F(SigningKeysTest, sign)
{
    ...
    for (auto const keyType : kKeyTypes)
    {
        ...
        auto const sig = strUnHex(keys.sign(data));
        ASSERT_TRUE(sig);
        ...
        EXPECT_EQ(errorOf([&] { external.sign(data); }), "This key file cannot be used to sign.");
        EXPECT_EQ(
            errorOf([&] { external.signHex(strHex(data)); }),
            "This key file cannot be used to sign.");
    }
    SigningKeys const keys(KeyType::Ed25519);
    EXPECT_EQ(errorOf([&] { keys.signHex("zz"); }), "Could not decode hex string: zz");
}
...
TEST_F(SigningKeysTest, external_master)
{
    for (auto const keyType : kKeyTypes)
    {
        ...
        for (auto const tokenKeyType : kKeyTypes)
        {
            ...
            auto const finished = fileKeys.finishToken(*strUnHex(signer.signHex(data)));
            ASSERT_TRUE(finished.secret);
            auto const m = manifestOf(finished.manifest, keys);
            EXPECT_EQ(m.sequence, ++sequence);
            EXPECT_EQ(fileKeys.sequence(), sequence);
            EXPECT_EQ(*m.signingKey, derivePublicKey(tokenKeyType, *finished.secret));

            // A signature over other bytes does not finish the token
            EXPECT_EQ(
                errorOf([&] { keys.finishToken(*strUnHex(signer.sign("foo"))); }), kBadManifest);
            EXPECT_EQ(
                errorOf([&] { keys.finishToken(*strUnHex(signer.signHex(data)), Blob{}); }),
                "The pending token's signing key is in this key file; pass one signature");
            keys.finishToken(*strUnHex(signer.signHex(data)));
        }
        ...
        EXPECT_EQ(errorOf([&] { keys.finishRevoke(*strUnHex(signer.sign("foo"))); }), kBadManifest);
        EXPECT_FALSE(keys.revoked());
        auto const sig = *strUnHex(signer.signHex(revocation));
...
TEST_F(SigningKeysTest, external_signing_key)
{
    ...
    auto const masterSig = *strUnHex(keys.signHex(data));
    auto const signingSig = *strUnHex(signer.signHex(data));
    EXPECT_EQ(
    ...
    EXPECT_EQ(*m.signingKey, signer.publicKey());
