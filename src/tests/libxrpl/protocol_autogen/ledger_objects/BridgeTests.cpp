...
TEST(BridgeTests, BuilderSettersRoundTrip)
{
...

    {
        ...
        auto const actual = entry->getXChainAccountClaimCount();
        expectEqualField(expected, actual, "sfXChainAccountClaimCount");
    }
...
TEST(BridgeTests, BuilderFromSleRoundTrip)
{
    ...
    SLE sle{Bridge::entryType, index};

    sle[sfAccount] = accountValue;
    sle[sfSignatureReward] = signatureRewardValue;
    sle[sfMinAccountCreateAmount] = minAccountCreateAmountValue;
    sle[sfXChainBridge] = xChainBridgeValue;
    sle[sfXChainClaimID] = xChainClaimIDValue;
    sle[sfXChainAccountCreateCount] = xChainAccountCreateCountValue;
    sle[sfXChainAccountClaimCount] = xChainAccountClaimCountValue;
...

    {
        ...
        auto const fromSle = entryFromSle.getXChainAccountClaimCount();
        auto const fromBuilder = entryFromBuilder->getXChainAccountClaimCount();

        expectEqualField(expected, fromSle, "sfXChainAccountClaimCount");
        expectEqualField(expected, fromBuilder, "sfXChainAccountClaimCount");
    }
