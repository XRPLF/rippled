...
#include <xrpl/protocol_autogen/ledger_objects/MPToken.h>
...
TEST(MPTokenTests, BuilderFromSleRoundTrip)
{
    ...
    SLE sle{MPToken::entryType, index};

    sle[sfAccount] = accountValue;
    sle[sfMPTokenIssuanceID] = mPTokenIssuanceIDValue;
    sle[sfMPTAmount] = mPTAmountValue;
...
TEST(MPTokenTests, WrapperThrowsOnWrongEntryType)
{
    ...
    EXPECT_THROW(MPToken{wrongEntry.object()}, std::runtime_error);
}
...
TEST(MPTokenTests, BuilderThrowsOnWrongEntryType)
{
    ...
    EXPECT_THROW(MPTokenBuilder{wrongEntry.object()}, std::runtime_error);
}
