...
#include <xrpl/protocol_autogen/ledger_objects/NFTokenPage.h>
...
TEST(NFTokenPageTests, BuilderFromSleRoundTrip)
{
    ...
    SLE sle{NFTokenPage::entryType, index};

    sle[sfPreviousPageMin] = previousPageMinValue;
    sle[sfNextPageMin] = nextPageMinValue;
...
TEST(NFTokenPageTests, WrapperThrowsOnWrongEntryType)
{
    ...
    EXPECT_THROW(NFTokenPage{wrongEntry.object()}, std::runtime_error);
}
...
TEST(NFTokenPageTests, BuilderThrowsOnWrongEntryType)
{
    ...
    EXPECT_THROW(NFTokenPageBuilder{wrongEntry.object()}, std::runtime_error);
}
