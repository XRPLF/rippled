...
#include <xrpl/protocol_autogen/ledger_objects/PermissionedDomain.h>
...
TEST(PermissionedDomainTests, BuilderFromSleRoundTrip)
{
    ...
    SLE sle{PermissionedDomain::entryType, index};
...
TEST(PermissionedDomainTests, WrapperThrowsOnWrongEntryType)
{
    ...
    EXPECT_THROW(PermissionedDomain{wrongEntry.object()}, std::runtime_error);
}
...
TEST(PermissionedDomainTests, BuilderThrowsOnWrongEntryType)
{
    ...
    EXPECT_THROW(PermissionedDomainBuilder{wrongEntry.object()}, std::runtime_error);
}
