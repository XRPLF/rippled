// Auto-generated unit tests for transaction ContractModify
...
TEST(TransactionsContractModifyTests, BuilderSettersRoundTrip)
{
...
TEST(TransactionsContractModifyTests, BuilderFromStTxRoundTrip)
{
    // Generate a deterministic keypair for signing
    auto const [publicKey, secretKey] =
        generateKeyPair(KeyType::secp256k1, generateSeed("testContractModifyFromTx"));
    ...
    ContractModifyBuilder initialBuilder{
        accountValue,
        sequenceValue,
        feeValue
    };
    ...
    ContractModifyBuilder builderFromTx{initialTx.getSTTx()};
...
TEST(TransactionsContractModifyTests, WrapperThrowsOnWrongTxType)
{
    ...
    EXPECT_THROW(ContractModify{wrongTx.getSTTx()}, std::runtime_error);
}

// 4) Verify builder throws when constructed from wrong transaction type.
TEST(TransactionsContractModifyTests, BuilderThrowsOnWrongTxType)
{
    // Build a valid transaction of a different type
    auto const [pk, sk] =
    ...
    EXPECT_THROW(ContractModifyBuilder{wrongTx.getSTTx()}, std::runtime_error);
}

// 5) Build with only required fields and verify optional fields return nullopt.
TEST(TransactionsContractModifyTests, OptionalFieldsReturnNullopt)
{
    // Generate a deterministic keypair for signing
    auto const [publicKey, secretKey] =
