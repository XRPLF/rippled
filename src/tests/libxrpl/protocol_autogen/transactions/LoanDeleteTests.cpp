...
TEST(TransactionsLoanDeleteTests, BuilderSettersRoundTrip)
{
...
TEST(TransactionsLoanDeleteTests, BuilderFromStTxRoundTrip)
{
    // Generate a deterministic keypair for signing
    auto const [publicKey, secretKey] =
    ...
    LoanDeleteBuilder initialBuilder{
        accountValue,
        sequenceValue,
        feeValue
    };
    ...
    LoanDeleteBuilder builderFromTx{initialTx.getSTTx()};
...
TEST(TransactionsLoanDeleteTests, WrapperThrowsOnWrongTxType)
{
    ...
    EXPECT_THROW(LoanDelete{wrongTx.getSTTx()}, std::runtime_error);
}

// 4) Verify builder throws when constructed from wrong transaction type.
TEST(TransactionsLoanDeleteTests, BuilderThrowsOnWrongTxType)
{
    // Build a valid transaction of a different type
    auto const [pk, sk] =
...
TEST(TransactionsLoanDeleteTests, OptionalFieldsReturnNullopt)
{
    // Generate a deterministic keypair for signing
    auto const [publicKey, secretKey] =
        generateKeyPair(KeyType::secp256k1, generateSeed("testLoanDeleteNullopt"));
