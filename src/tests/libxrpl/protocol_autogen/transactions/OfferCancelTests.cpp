...
TEST(TransactionsOfferCancelTests, BuilderFromStTxRoundTrip)
{
    ...
    OfferCancelBuilder builderFromTx{initialTx.getSTTx()};
...
TEST(TransactionsOfferCancelTests, WrapperThrowsOnWrongTxType)
{
    ...
    EXPECT_THROW(OfferCancel{wrongTx.getSTTx()}, std::runtime_error);
}

// 4) Verify builder throws when constructed from wrong transaction type.
TEST(TransactionsOfferCancelTests, BuilderThrowsOnWrongTxType)
{
    // Build a valid transaction of a different type
    auto const [pk, sk] =
    ...
    EXPECT_THROW(OfferCancelBuilder{wrongTx.getSTTx()}, std::runtime_error);
}

// 5) Build with only required fields and verify optional fields return nullopt.
TEST(TransactionsOfferCancelTests, OptionalFieldsReturnNullopt)
{
    // Generate a deterministic keypair for signing
    auto const [publicKey, secretKey] =
        generateKeyPair(KeyType::secp256k1, generateSeed("testOfferCancelNullopt"));
