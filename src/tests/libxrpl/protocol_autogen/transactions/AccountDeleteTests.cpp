...
TEST(TransactionsAccountDeleteTests, BuilderFromStTxRoundTrip)
{
    ...
    AccountDeleteBuilder builderFromTx{initialTx.getSTTx()};
...
TEST(TransactionsAccountDeleteTests, WrapperThrowsOnWrongTxType)
{
    ...
    EXPECT_THROW(AccountDelete{wrongTx.getSTTx()}, std::runtime_error);
}

// 4) Verify builder throws when constructed from wrong transaction type.
TEST(TransactionsAccountDeleteTests, BuilderThrowsOnWrongTxType)
{
    // Build a valid transaction of a different type
    auto const [pk, sk] =
    ...
    EXPECT_THROW(AccountDeleteBuilder{wrongTx.getSTTx()}, std::runtime_error);
}

// 5) Build with only required fields and verify optional fields return nullopt.
TEST(TransactionsAccountDeleteTests, OptionalFieldsReturnNullopt)
{
    // Generate a deterministic keypair for signing
    auto const [publicKey, secretKey] =
