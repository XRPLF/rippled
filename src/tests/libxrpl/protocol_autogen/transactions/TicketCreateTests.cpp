...
TEST(TransactionsTicketCreateTests, BuilderSettersRoundTrip)
{
    ...
    // Verify required fields
    {
        ...
        auto const actual = tx->getTicketCount();
        expectEqualField(expected, actual, "sfTicketCount");
    }
...
TEST(TransactionsTicketCreateTests, BuilderFromStTxRoundTrip)
{
    ...
    TicketCreateBuilder builderFromTx{initialTx.object()};
    ...
    {
        auto const& expected = ticketCountValue;
        auto const actual = rebuiltTx->getTicketCount();
        expectEqualField(expected, actual, "sfTicketCount");
    }
...
TEST(TransactionsTicketCreateTests, WrapperThrowsOnWrongTxType)
{
    ...
    EXPECT_THROW(TicketCreate{wrongTx.object()}, std::runtime_error);
}
...
TEST(TransactionsTicketCreateTests, BuilderThrowsOnWrongTxType)
{
    ...
    EXPECT_THROW(TicketCreateBuilder{wrongTx.object()}, std::runtime_error);
}
