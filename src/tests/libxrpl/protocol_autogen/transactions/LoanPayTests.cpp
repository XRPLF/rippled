...
TEST(TransactionsLoanPayTests, BuilderSettersRoundTrip)
{
...
TEST(TransactionsLoanPayTests, BuilderFromStTxRoundTrip)
{
    // Generate a deterministic keypair for signing
    auto const [publicKey, secretKey] =
    ...
    LoanPayBuilder builderFromTx{initialTx.getSTTx()};
...
TEST(TransactionsLoanPayTests, WrapperThrowsOnWrongTxType)
{
    ...
    EXPECT_THROW(LoanPay{wrongTx.getSTTx()}, std::runtime_error);
}

// 4) Verify builder throws when constructed from wrong transaction type.
TEST(TransactionsLoanPayTests, BuilderThrowsOnWrongTxType)
{
    // Build a valid transaction of a different type
    auto const [pk, sk] =
...
TEST(TransactionsLoanPayTests, OptionalFieldsReturnNullopt)
{
    // Generate a deterministic keypair for signing
    auto const [publicKey, secretKey] =
        generateKeyPair(KeyType::secp256k1, generateSeed("testLoanPayNullopt"));
