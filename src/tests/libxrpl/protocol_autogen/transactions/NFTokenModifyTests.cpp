...
#include <xrpl/protocol_autogen/transactions/NFTokenModify.h>
...
TEST(TransactionsNFTokenModifyTests, BuilderSettersRoundTrip)
{
    ...
    // Verify optional fields
    {
        ...
        auto const actualOpt = tx.getNFTokenID();
        ASSERT_TRUE(actualOpt.has_value()) << "Optional field sfNFTokenID should be present";
        expectEqualField(expected, *actualOpt, "sfNFTokenID");
        EXPECT_TRUE(tx.hasNFTokenID());
    }
...
TEST(TransactionsNFTokenModifyTests, BuilderFromStTxRoundTrip)
{
    // Generate a deterministic keypair for signing
    auto const [publicKey, secretKey] =
    ...
    NFTokenModifyBuilder initialBuilder{
        accountValue,
        sequenceValue,
        feeValue
    };

    initialBuilder.setNFTokenID(nFTokenIDValue);
    ...
    {
        auto const& expected = nFTokenIDValue;
        auto const actualOpt = rebuiltTx.getNFTokenID();
        ASSERT_TRUE(actualOpt.has_value()) << "Optional field sfNFTokenID should be present";
        expectEqualField(expected, *actualOpt, "sfNFTokenID");
    }
...
TEST(TransactionsNFTokenModifyTests, WrapperThrowsOnWrongTxType)
{
...
TEST(TransactionsNFTokenModifyTests, BuilderThrowsOnWrongTxType)
{
    // Build a valid transaction of a different type
    auto const [pk, sk] =
...
TEST(TransactionsNFTokenModifyTests, OptionalFieldsReturnNullopt)
{
    // Generate a deterministic keypair for signing
    auto const [publicKey, secretKey] =
    ...
    EXPECT_FALSE(tx.hasNFTokenID());
    EXPECT_FALSE(tx.getNFTokenID().has_value());
