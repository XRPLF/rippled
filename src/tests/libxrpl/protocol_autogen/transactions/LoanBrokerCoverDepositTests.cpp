...
TEST(TransactionsLoanBrokerCoverDepositTests, WrapperThrowsOnWrongTxType)
{
    ...
    EXPECT_THROW(LoanBrokerCoverDeposit{wrongTx.object()}, std::runtime_error);
}

// 4) Verify builder throws when constructed from wrong transaction type.
TEST(TransactionsLoanBrokerCoverDepositTests, BuilderThrowsOnWrongTxType)
{
    // Build a valid transaction of a different type
    auto const [pk, sk] =
    ...
    EXPECT_THROW(LoanBrokerCoverDepositBuilder{wrongTx.object()}, std::runtime_error);
}
