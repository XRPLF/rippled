...
// nft_offer_id — an account region and a four-byte `seq` region in, a keylet region out.
struct NftokenOfferKeyletGuest : GuestCallTest
{
    ...
    static constexpr std::uint32_t kSeqValue = 0x4d5e6f70;
    ...
    Bytes const accountBytes = Bytes(AccountID::size(), 0x4f);
