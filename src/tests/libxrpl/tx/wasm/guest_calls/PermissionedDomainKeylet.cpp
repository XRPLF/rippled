...
// permissioned_domain_id — an account region and a four-byte `seq` region in, a keylet region out.
struct PermissionedDomainKeyletGuest : GuestCallTest
{
    ...
    static constexpr std::uint32_t kSeqValue = 0x5e6f7081;
    ...
    Bytes const accountBytes = Bytes(AccountID::size(), 0xbd);
