...
TEST_F(Sha512HalfCall, DataForwardedAndDigestWritten)
{
...
TEST_F(Sha512HalfCall, HostExceptionBecomesInternalFatalAndIsLogged)
{
    EXPECT_CALL(host, computeSha512HalfHash)
...
// either way.
TEST_F(Sha512HalfCall, ShortOutRegionWritesNothingAndReturnsTrueLength)
{
    EXPECT_CALL(host, computeSha512HalfHash(BytesAre("abc"))).WillOnce(testing::Return(digest));
...
// not refused.
TEST_F(Sha512HalfCall, EmptyInputIsHashedLikeAnyOther)
{
