...
#define LEB() parseLeb128(hook, i, &i)

#define SIGNED_LEB() parseSignedLeb128(hook, i, &i)
...
inline uint64_t
compute_wce(const WasmBlkInf* blk, int level = 0)
{
    ...
    std::string guardLogAccStr)
{
    ...
    for (int i = start_offset; i < end_offset;)
    {
            ...
            instr == 0x04U)    // if
        {
            ...
            uint32_t iteration_bound =
                (current->parent == 0 ? 1 : current->parent->iteration_bound);
            if (instr == 0x03U)
            {
    ...
    std::string guardLogAccStr)
{
    ...
    for (int i = 8, j = 0; i < hook.size();)
    {
        ...
        // we are interested in the import section... we need to know if _g is
        // imported and which import# it is
        if (section_type == 2)  // import section
        {
            import_count = parseLeb128(hook, i, &i);
            CHECK_SHORT_HOOK();
            if (import_count <= 0)
            {
            ...
            for (int j = 0; j < import_count; ++j)
            {
                ...
                int mod_length = parseLeb128(hook, i, &i);
                CHECK_SHORT_HOOK();
                ...
                if (std::string_view(
                        (const char*)(hook.data() + i), (size_t)mod_length) !=
                    "env")
                {
                ...
                int name_length = parseLeb128(hook, i, &i);
                CHECK_SHORT_HOOK();
                ...
                std::string import_name{
                    (const char*)(hook.data() + i), (size_t)name_length};
                ...
                // RH TODO: validate that the parameters of the imported
        ...
        else if (section_type == 7)  // export section
        {
            ...
            for (int j = 0; j < export_count; ++j)
            {
                ...
                if (name_len == 4)
                {
                    if (hook[i] == 'h' && hook[i + 1] == 'o' &&
                        hook[i + 2] == 'o' && hook[i + 3] == 'k')
                    {
                        i += name_len;
                        CHECK_SHORT_HOOK();
                        if (hook[i] != 0)
                        ...
                        i++;
                        CHECK_SHORT_HOOK();
                        hook_func_idx = parseLeb128(hook, i, &i);
                        CHECK_SHORT_HOOK();
                        continue;
                    }

                    if (hook[i] == 'c' && hook[i + 1] == 'b' &&
                        hook[i + 2] == 'a' && hook[i + 3] == 'k')
                    {
                        i += name_len;
                        CHECK_SHORT_HOOK();
                        if (hook[i] != 0)
                        ...
                        i++;
                        CHECK_SHORT_HOOK();
                        cbak_func_idx = parseLeb128(hook, i, &i);
                        CHECK_SHORT_HOOK();
                        continue;
                    }
                }
        ...
        {
            int function_count = parseLeb128(hook, i, &i);
            CHECK_SHORT_HOOK();
            if (function_count <= 0)
            {
            ...
            {
                int type_idx = parseLeb128(hook, i, &i);
                CHECK_SHORT_HOOK();
    ...
    */
    ...
    for (int i = 8; i < hook.size();)
    {
        ...
        {
            int type_count = parseLeb128(hook, i, &i);
            CHECK_SHORT_HOOK();
            for (int j = 0; j < type_count; ++j)
            {
                ...
                int param_count = parseLeb128(hook, i, &i);
                CHECK_SHORT_HOOK();
                ...
                {
                    int param_type = parseLeb128(hook, i, &i);
                    CHECK_SHORT_HOOK();
                    ...
                    if (j == hook_type_idx && param_type != 0x7FU /* i32 */)
                    {
                        GUARDLOG(hook::log::PARAM_HOOK_CBAK)
                            << "Malformed transaction. "
                            << "hook and cbak function definition must have "
                               "exactly one uint32_t parameter."
                            << "\n";
                        return {};
                    }
                }

                int result_count = parseLeb128(hook, i, &i);
                CHECK_SHORT_HOOK();
                ...
                {
                    int result_type = parseLeb128(hook, i, &i);
                    CHECK_SHORT_HOOK();
        ...
        else if (section_type == 10)  // code section
        {
            ...
            int func_count = parseLeb128(hook, i, &i);
            CHECK_SHORT_HOOK();

            for (int j = 0; j < func_count; ++j)
            {
                // parse locals
                int code_size = parseLeb128(hook, i, &i);
                CHECK_SHORT_HOOK();
                int code_end = i + code_size;
                int local_count = parseLeb128(hook, i, &i);
                CHECK_SHORT_HOOK();
