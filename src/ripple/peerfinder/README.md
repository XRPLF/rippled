...
of the Manager, usually from the configuration file.
...
The Bootstrap strategy is invoked when there are no active peer connections.

## Connection Strategy

The connection strategy is used when there are active peer connections but the
number of active outbound connections is below the desired amount.

## Inbound Strategy

The inbound strategy controls the way that inbound peer connection requests are
handled.
