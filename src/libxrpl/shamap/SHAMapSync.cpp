    ...
    const
{
    visitNodes([&leafFunction](SHAMapTreeNode& node) {
        if (!node.isInner())
            leafFunction(dynamic_cast<SHAMapLeafNode&>(node).peekItem());
        return true;
    });
}
    ...
    std::function<bool(SHAMapTreeNode const&)> const& function) const
{
    ...
    stack.emplace(dynamic_cast<SHAMapInnerNode*>(root_.get()), SHAMapNodeID{});
            ...
            if (!node->isEmptyBranch(i))
            {
                ...
                if (next->isInner())
                {
                    if ((have == nullptr) || !have->hasInnerNode(childID, childHash))
                        stack.emplace(dynamic_cast<SHAMapInnerNode*>(next), childID);
                }
                else if (
                    (have == nullptr) ||
                    !have->hasLeafNode(
                        dynamic_cast<SHAMapLeafNode*>(next)->peekItem()->key(), childHash))
                {
                    if (!function(*next))
                        return;
                }
    ...
    while (currentChild < 16)
    {
        ...
        else if (!backed_ || !f_.getFullBelowCache()->touch_if_exists(childHash.as_uint256()))
        {
            ...
            else if (d == nullptr)
            {
                ...
                d->isInner() && !dynamic_cast<SHAMapInnerNode*>(d)->isFullBelow(mn.generation_))
            {
                mn.stack_.push(se);

                // Switch to processing the child node
                node = dynamic_cast<SHAMapInnerNode*>(d);
...
SHAMap::getMissingNodes(int max, SHAMapSyncFilter* filter)
{
    ...
    MissingNodes::StackEntry pos{
        dynamic_cast<SHAMapInnerNode*>(root_.get()), SHAMapNodeID(), rand_int(255), 0, true};
    ...
    std::uint32_t depth) const
{
    ...
    if (node->isInner() && dynamic_cast<SHAMapInnerNode*>(node)->isEmpty())
    {
...
SHAMap::addKnownNode(SHAMapNodeID const& node, Slice const& rawNode, SHAMapSyncFilter* filter)
{
    ...
    while (currNode->isInner() &&
           !dynamic_cast<SHAMapInnerNode*>(currNode)->isFullBelow(generation) &&
           (currNodeID.getDepth() < node.getDepth()))
    {
        ...
        auto inner = dynamic_cast<SHAMapInnerNode*>(currNode);
        if (inner->isEmptyBranch(branch))
        {
        ...
        if (newNode->isLeaf())
        {
            auto const& actualKey =
                dynamic_cast<SHAMapLeafNode const*>(newNode.get())->peekItem()->key();
...
SHAMap::deepCompare(SHAMap& other) const
{
    ...
    while (!stack.empty())
    {
        ...
        if (node->isLeaf())
        {
            if (!otherNode->isLeaf())
                return false;
            auto& nodePeek = dynamic_cast<SHAMapLeafNode*>(node)->peekItem();
            auto& otherNodePeek = dynamic_cast<SHAMapLeafNode*>(otherNode)->peekItem();
        ...
        else if (node->isInner())
        {
            if (!otherNode->isInner())
                return false;
            auto node_inner = dynamic_cast<SHAMapInnerNode*>(node);
            auto other_inner = dynamic_cast<SHAMapInnerNode*>(otherNode);
            for (int i = 0; i < 16; ++i)
            {
    ...
    try
    {
        for (auto rit = path.rbegin(); rit != path.rend(); ++rit)
        {
            ...
            if (node->isInner())
            {
                ...
                hash = dynamic_cast<SHAMapInnerNode*>(node.get())
