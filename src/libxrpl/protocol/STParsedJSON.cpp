    ...
    Json::Value& error)
{
    ...
    try
    {
        ...
        {
            error = bad_type(jsonName, fieldName);
            return ret;
        }
    }
    catch (std::exception const&)
    {
        error = invalid_data(jsonName, fieldName);
        return ret;
    }
    ...
    Json::Value& error)
{
    ...
    {
        // LCOV_EXCL_START
        error = unknown_field(jsonName, fieldName);
        return ret;
        // LCOV_EXCL_STOP
    }
    ...
    switch (field.fieldType)
    {
                ...
                if (value.isString())
                {
                    ...
                    if (!strValue.empty() && ((strValue[0] < '0') || (strValue[0] > '9')))
                    {
                        if (field == sfTransactionResult)
                        {
                            ...
                            {
                                error = out_of_range(jsonName, fieldName);
                                return ret;
                            }
        ...
        case STI_VECTOR256:
            ...
            {
                error = array_expected(jsonName, fieldName);
                return ret;
            }
            ...
            try
            {
                ...
                for (Json::UInt i = 0; value.isValidIndex(i); ++i)
                {
                    ...
                    if (!value[i].isArrayOrNull())
                    {
                        std::stringstream ss;
                        ss << fieldName << "[" << i << "]";
                        error = array_expected(jsonName, ss.str());
                        return ret;
                    }
                    ...
                    for (Json::UInt j = 0; value[i].isValidIndex(j); ++j)
                    {
                        ...
                        if (!pathEl.isObject())
                        {
                            error = not_an_object(elementName);
                            return ret;
                        }
                        ...
                        {
                            error = invalid_data(elementName);
                            return ret;
                        }
                        ...
                        if (account)
                        {
                            ...
                            if (!account.isString())
                            {
                                error = string_expected(elementName, "account");
                                return ret;
                            }
                            ...
                            if (!uAccount.parseHex(account.asString()))
                            {
                                ...
                                if (!a)
                                {
                                    error = invalid_data(elementName, "account");
                                    return ret;
                                }
                                uAccount = *a;
                            }
                        ...
                        if (currency)
                        {
                            ...
                            {
                                error = string_expected(elementName, "currency");
                                return ret;
                            }
                            ...
                            if (!uCurrency.parseHex(currency.asString()))
                            {
                                ...
                                {
                                    error = invalid_data(elementName, "currency");
                                    return ret;
                                }
                            }
                        }
                        ...
                        if (issuer)
                        {
                            ...
                            if (!issuer.isString())
                            {
                                error = string_expected(elementName, "issuer");
                                return ret;
                            }
                            ...
                            if (!uIssuer.parseHex(issuer.asString()))
                            {
                                ...
                                if (!a)
                                {
                                    error = invalid_data(elementName, "issuer");
                                    return ret;
                                }
                                uIssuer = *a;
                            }
    ...
    Json::Value& error)
{
    ...
    {
        error = not_an_object(jsonName);
        return std::nullopt;
    }

    if (depth > maxDepth)
    {
        error = too_deep(jsonName);
        return std::nullopt;
    }
        ...
        for (auto const& fieldName : json.getMemberNames())
        {
            ...
            switch (field.fieldType)
            {
                ...
                case STI_VALIDATION:
                    if (!value.isObject())
                    {
                        error = not_an_object(jsonName, fieldName);
                        return std::nullopt;
                    }
    ...
    try
    {
        ...
        for (Json::UInt i = 0; json.isValidIndex(i); ++i)
        {
            ...
            if (!isObjectOrNull || !singleKey)
            {
                ...
                error = singleton_expected(jsonName, i);
                return std::nullopt;
            }
            ...
            {
                error = unknown_field(jsonName, objectName);
                return std::nullopt;
            }
