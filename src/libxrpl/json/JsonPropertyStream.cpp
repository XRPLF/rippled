...
JsonPropertyStream::JsonPropertyStream() : topValue(json::ValueType::ObjectValue)
{
    stack.reserve(64);
...
JsonPropertyStream::mapBegin()
{
    ...
    json::Value& map(top.append(json::ValueType::ObjectValue));
    stack.push_back(&map);
}
...
JsonPropertyStream::mapBegin(std::string const& key)
{
    // top is a map
    json::Value& top(*stack.back());
    json::Value& map(top[key] = json::ValueType::ObjectValue);
    stack.push_back(&map);
}
...
JsonPropertyStream::arrayBegin()
{
    // top is array
    json::Value& top(*stack.back());
    json::Value& vec(top.append(json::ValueType::ArrayValue));
    stack.push_back(&vec);
}
...
JsonPropertyStream::arrayBegin(std::string const& key)
{
    // top is a map
    json::Value& top(*stack.back());
    json::Value& vec(top[key] = json::ValueType::ArrayValue);
    stack.push_back(&vec);
}
