...
TrustSet::preclaim(PreclaimContext const& ctx)
{
    ...
    if (sleDst->isFlag(lsfDisallowIncomingTrustline))
    {
        ...
        if (ctx.view.rules().enabled(fixDisallowIncomingV1) &&
            ctx.view.exists(keylet::rippleState(id, uDstAccountID, currency)))
        {
            // pass
        }
        else
    ...
    if (sleDst && isPseudoAccount(sleDst))
    {
        ...
        {
            if (ctx.view.exists(keylet::rippleState(id, uDstAccountID, currency)))
            {
                // pass
            }
    ...
    if (ctx.view.rules().enabled(featureDeepFreeze))
    {
        ...
        auto const sleRippleState = ctx.view.read(keylet::rippleState(id, uDstAccountID, currency));
...
TrustSet::doApply()
{
    ...
    SLE::pointer const sleRippleState =
        view().peek(keylet::rippleState(accountID_, uDstAccountID, currency));
    ...
    else
    {
        ...
        auto const k = keylet::rippleState(accountID_, uDstAccountID, currency);
