        ...
        [](ApplyView& view,
           ...
           std::optional<AccountID> const& sponsor) -> TER {
            ...
            if (isReserveSponsored(tx))
            {
                ...
                if (auto const ret = checkInsufficientReserve(view, tx, ownerSle, ownerBalance, sponsorSle, 1);
                    !isTesSuccess(ret))
                    return ret;
            }
            adjustOwnerCount(
    ...
    std::uint32_t txFlags)
{
    ...
    if (auto const ret = checkInsufficientReserve(view, tx, acct, priorBalance, sponsor, 1); !isTesSuccess(ret))
        return ret;
