...
NFTokenAcceptOffer::preclaim(PreclaimContext const& ctx)
{
    ...
    if (bo)
    {
        ...
        if (accountFunds(ctx.view, (*bo)[sfOwner], needed, FhZeroIfFrozen, ctx.j) < needed)
    ...
    if (so)
    {
        ...
        if (!bo)
        {
            ...
            if (accountFunds(ctx.view, ctx.tx[sfAccount], needed, FhZeroIfFrozen, ctx.j) < needed)
                return tecINSUFFICIENT_FUNDS;
        }
...
NFTokenAcceptOffer::pay(AccountID const& from, AccountID const& to, STAmount const& amount)
{
    ...
    if (!isTesSuccess(result))
        return result;
    if (accountFunds(view(), from, amount, FhZeroIfFrozen, j_).signum() < 0)
        return tecINSUFFICIENT_FUNDS;
    if (accountFunds(view(), to, amount, FhZeroIfFrozen, j_).signum() < 0)
        return tecINSUFFICIENT_FUNDS;
    return tesSUCCESS;
}
