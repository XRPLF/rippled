...
VaultCreate::doApply()
{
    ...
    if (auto ter = addEmptyHolding(view(), pseudoId, preFeeBalance, asset, j_); !isTesSuccess(ter))
        return ter;
    ...
    if (auto const err =
            authorizeMPToken(view(), preFeeBalance, mptIssuanceID, account_, ctx_.journal);
        !isTesSuccess(err))
        return err;
    ...
    if (txFlags & tfVaultPrivate)
    {
        if (auto const err = authorizeMPToken(
                view(), preFeeBalance, mptIssuanceID, pseudoId, ctx_.journal, {}, account_);
            !isTesSuccess(err))
            return err;
    }
