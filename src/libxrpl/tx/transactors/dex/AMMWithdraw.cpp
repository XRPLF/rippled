...
#include <xrpl/tx/transactors/dex/AMMWithdraw.h>
    ...
    namespace xrpl
{
    bool AMMWithdraw::checkExtraFeatures(PreflightContext const& ctx)
    {
        return ammEnabled(ctx.rules);
    }

    std::uint32_t AMMWithdraw::getFlagsMask(PreflightContext const& ctx)
    {
        return tfAMMWithdrawMask;
    }

    NotTEC AMMWithdraw::preflight(PreflightContext const& ctx)
    {
    ...
    static std::optional<STAmount> tokensWithdraw(
        STAmount const& lpTokens, std::optional<STAmount> const& tokensIn, std::uint32_t flags)
    {
    ...
    TER AMMWithdraw::preclaim(PreclaimContext const& ctx)
    {
        ...
        auto checkAmount = [&](std::optional<STAmount> const& amount, auto const& balance) -> TER {
            if (amount)
            {
                ...
                {
                    JLOG(ctx.j.debug()) << "AMM Withdraw: AMM account or currency is frozen, "
                                        << to_string(accountID);
                    return tecFROZEN;
                }
                // Account frozen
                ...
                {
                    JLOG(ctx.j.debug())
                        << "AMM Withdraw: account is frozen, " << to_string(accountID) << " "
    ...
    std::pair<TER, bool> AMMWithdraw::applyGuts(Sandbox & sb)
    {
        ...
        {
            if (auto const res = verifyAndAdjustLPTokenBalance(sb, lpTokens, ammSle, account_);
                !res)
    ...
    std::pair<TER, STAmount> AMMWithdraw::withdraw(
        Sandbox & view,
        SLE const& ammSle,
    ...
    std::tuple<TER, STAmount, STAmount, std::optional<STAmount>> AMMWithdraw::withdraw(
        Sandbox & view,
        SLE const& ammSle,
        AccountID const& ammAccount,
        ...
        beast::Journal const& journal)
    {
        ...
        auto const expected =
            ammHolds(view, ammSle, amountWithdraw.issue(), std::nullopt, freezeHandling, journal);
        // LCOV_EXCL_START
        if (!expected)
    ...
    static STAmount adjustLPTokensIn(
        Rules const& rules,
     ...
     */
    std::pair<TER, STAmount> AMMWithdraw::equalWithdrawTokens(
        Sandbox & view,
        SLE const& ammSle,
        AccountID const& ammAccount,
        STAmount const& amountBalance,
    ...
    std::pair<TER, bool> AMMWithdraw::deleteAMMAccountIfEmpty(
        Sandbox & sb,
    ...
    /** Proportional withdrawal of pool assets for the amount of LPTokens.
     */
    std::tuple<TER, STAmount, STAmount, std::optional<STAmount>> AMMWithdraw::equalWithdrawTokens(
        Sandbox & view,
        SLE const& ammSle,
        AccountID const account,
        AccountID const& ammAccount,
     ...
     */
    std::pair<TER, STAmount> AMMWithdraw::equalWithdrawLimit(
        Sandbox & view,
        SLE const& ammSle,
        AccountID const& ammAccount,
        STAmount const& amountBalance,
     ...
     */
    std::pair<TER, STAmount> AMMWithdraw::singleWithdraw(
        Sandbox & view,
        SLE const& ammSle,
        AccountID const& ammAccount,
        STAmount const& amountBalance,
     ...
     */
    std::pair<TER, STAmount> AMMWithdraw::singleWithdrawTokens(
        Sandbox & view,
        SLE const& ammSle,
        AccountID const& ammAccount,
        STAmount const& amountBalance,
     ...
     */
    std::pair<TER, STAmount> AMMWithdraw::singleWithdrawEPrice(
        Sandbox & view,
        SLE const& ammSle,
        AccountID const& ammAccount,
        STAmount const& amountBalance,
    ...
    WithdrawAll AMMWithdraw::isWithdrawAll(STTx const& tx)
    {
