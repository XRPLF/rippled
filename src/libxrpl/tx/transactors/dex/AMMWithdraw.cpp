    ...
    beast::Journal const& journal)
{
    ...
    auto sufficientReserve = [&](Asset const& asset) -> TER {
        ...
        if (assetNotExists)
        {
            auto wrappedAccount = WAccountRoot(account, view, journal);
            if (!wrappedAccount)
                return tecINTERNAL;  // LCOV_EXCL_LINE
            STAmount const balance = (*wrappedAccount)[sfBalance];
            std::uint32_t const ownerCount = wrappedAccount->at(sfOwnerCount);
