...
#include <xrpl/tx/transactors/dex/AMMUtils.h>
    ...
    namespace xrpl
{
    bool AMMDeposit::checkExtraFeatures(PreflightContext const& ctx)
    {
        return ammEnabled(ctx.rules);
    }

    std::uint32_t AMMDeposit::getFlagsMask(PreflightContext const& ctx)
    {
    ...
    TER AMMDeposit::preclaim(PreclaimContext const& ctx)
    {
        ...
        auto checkAmount = [&](std::optional<STAmount> const& amount, bool checkBalance) -> TER {
            if (amount)
            {
                ...
                {
                    JLOG(ctx.j.debug()) << "AMM Deposit: AMM account or currency is frozen, "
                                        << to_string(accountID);
                    return tecFROZEN;
                }
                // Account frozen
                ...
                {
                    JLOG(ctx.j.debug())
                        << "AMM Deposit: account is frozen, " << to_string(accountID) << " "
    ...
    std::pair<TER, bool> AMMDeposit::applyGuts(Sandbox & sb)
    {
    ...
    std::pair<TER, STAmount> AMMDeposit::deposit(
        Sandbox & view,
        ...
        std::uint16_t tfee)
    {
        ...
        auto checkBalance = [&](auto const& depositAmount) -> TER {
            ...
            if (isXRP(depositAmount))
            {
                ...
                // Adjust the reserve if LP doesn't have LPToken trustline
                auto const sle =
                    view.read(keylet::line(account_, lpIssue.account, lpIssue.currency));
    ...
    static STAmount adjustLPTokensOut(
        Rules const& rules, STAmount const& lptAMMBalance, STAmount const& lpTokensDeposit)
    {
     ...
     * amount of LPTokens.
     */
    std::pair<TER, STAmount> AMMDeposit::equalDepositTokens(
        Sandbox & view,
        AccountID const& ammAccount,
        STAmount const& amountBalance,
     ...
     */
    std::pair<TER, STAmount> AMMDeposit::equalDepositLimit(
        Sandbox & view,
        AccountID const& ammAccount,
        STAmount const& amountBalance,
        STAmount const& amount2Balance,
     ...
     */
    std::pair<TER, STAmount> AMMDeposit::singleDeposit(
        Sandbox & view,
        AccountID const& ammAccount,
        STAmount const& amountBalance,
        STAmount const& lptAMMBalance,
     ...
     */
    std::pair<TER, STAmount> AMMDeposit::singleDepositTokens(
        Sandbox & view,
        AccountID const& ammAccount,
        STAmount const& amountBalance,
        STAmount const& amount,
     ...
     *   The amount of LPTokens to be issued is W
     */
    std::pair<TER, STAmount> AMMDeposit::singleDepositEPrice(
        Sandbox & view,
        AccountID const& ammAccount,
        STAmount const& amountBalance,
        STAmount const& amount,
    ...
    std::pair<TER, STAmount> AMMDeposit::equalDepositInEmptyState(
        Sandbox & view,
        AccountID const& ammAccount,
        STAmount const& amount,
        STAmount const& amount2,
