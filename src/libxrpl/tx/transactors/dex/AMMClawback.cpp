...
#include <tuple>
    ...
    namespace xrpl
{
    std::uint32_t AMMClawback::getFlagsMask(PreflightContext const& ctx)
    {
        return tfAMMClawbackMask;
    }

    NotTEC AMMClawback::preflight(PreflightContext const& ctx)
    {
    ...
    TER AMMClawback::preclaim(PreclaimContext const& ctx)
    {
        auto const asset = ctx.tx[sfAsset].get<Issue>();
        auto const asset2 = ctx.tx[sfAsset2].get<Issue>();
    ...
    TER AMMClawback::applyGuts(Sandbox & sb)
    {
        ...
        {
            // retrieve LP token balance inside the amendment gate to avoid inconsistent error
            // behavior
        ...
        // calling a second time on purpose since `verifyAndAdjustLPTokenBalance` rounds and may
        // adjust the balance
