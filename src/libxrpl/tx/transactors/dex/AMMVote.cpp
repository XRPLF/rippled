...
#include <xrpl/tx/transactors/dex/AMMVote.h>
    ...
    namespace xrpl
{
    bool AMMVote::checkExtraFeatures(PreflightContext const& ctx)
    {
        return ammEnabled(ctx.rules);
    }

    NotTEC AMMVote::preflight(PreflightContext const& ctx)
    {
        if (auto const res =
    ...
    TER AMMVote::preclaim(PreclaimContext const& ctx)
    {
    ...
    static std::pair<TER, bool> applyVote(
