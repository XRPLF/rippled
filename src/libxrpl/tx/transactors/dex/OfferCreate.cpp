...
<xrpl / ledger / helpers / PermissionedDEXHelpers.h>
    ...
    namespace xrpl
{
    TxConsequences OfferCreate::makeTxConsequences(PreflightContext const& ctx)
    {
    ...
    std::uint32_t OfferCreate::getFlagsMask(PreflightContext const& ctx)
    {
    ...
    NotTEC OfferCreate::preflight(PreflightContext const& ctx)
    {
        auto& tx = ctx.tx;
        auto& j = ctx.j;
    ...
    TER OfferCreate::preclaim(PreclaimContext const& ctx)
    {
        ...
        if (!saTakerPays.native())
        {
            auto result = checkAcceptAsset(
                ctx.view, ctx.flags, id, ctx.j, Issue(uPaysCurrency, uPaysIssuerID));
    ...
    TER OfferCreate::checkAcceptAsset(
        ReadView const& view,
        ...
        Issue const& issue)
    {
        ...
        XRPL_ASSERT(
            !isXRP(issue.currency), "xrpl::OfferCreate::checkAcceptAsset : input is not XRP");
    ...
    std::pair<TER, Amounts> OfferCreate::flowCross(
        PaymentSandbox & psb,
            ...
            if (!sendMax.native() && (account_ != sendMax.getIssuer()))
            {
                ...
                if (gatewayXferRate.value != QUALITY_ONE)
                {
                    sendMax = multiplyRound(
                        takerAmount.in, gatewayXferRate, takerAmount.in.issue(), true);
                }
            }
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (afterCross.out < beast::zero)
                            afterCross.out.clear();
                        afterCross.in =
                            mulRound(afterCross.out, rate, takerAmount.in.issue(), true);
                    }
                }
            }
    ...
    std::string OfferCreate::format_amount(STAmount const& amount)
    {
    ...
    std::pair<TER, bool> OfferCreate::applyGuts(Sandbox & sb, Sandbox & sbCancel)
    {
        using beast::zero;
