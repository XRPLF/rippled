...
EscrowCreate::doApply()
{
    ...
    if (auto const ret = checkInsufficientReserve(ctx_.view(), ctx_.tx, sle, mSourceBalance, sponsor, 1);
        !isTesSuccess(ret))
        return ret;
    ...
    {
        if (auto const ret =
                checkInsufficientReserve(ctx_.view(), ctx_.tx, sle, mSourceBalance - STAmount(amount).xrp(), {}, 1);
            !isTesSuccess(ret))
            return tecUNFUNDED;
    }
    ...
    beast::Journal journal)
{
    ...
    if (!view.exists(trustLineKey) && createAsset && !receiverIssuer)
    {
        ...
        if (auto const ret = checkInsufficientReserve(view, tx, sleDest, xrpBalance, sponsorSle, 1); !isTesSuccess(ret))
        {
