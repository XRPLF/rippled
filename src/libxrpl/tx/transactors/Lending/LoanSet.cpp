...
LoanSet::doApply()
{
    ...
    auto const sponsorSle = getTxReserveSponsor(view, tx);
    {
        ...
        if (auto const ret = checkInsufficientReserve(view, tx, borrowerSle, balance, sponsorSle, 1);
            !isTesSuccess(ret))
            return ret;
    }
    ...
    if (auto const ter = addEmptyHolding(view, tx, borrower, borrowerSle->at(sfBalance).value().xrp(), vaultAsset, j_);
    ...
    if (originationFee != beast::zero)
    {
        ...
        if (auto const ter =
                addEmptyHolding(view, tx, brokerOwner, brokerOwnerSle->at(sfBalance).value().xrp(), vaultAsset, j_);
            ter && ter != tecDUPLICATE)
