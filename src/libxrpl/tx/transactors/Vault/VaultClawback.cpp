...
VaultClawback::doApply()
{
    ...
    if (auto const ter =
            accountSend(view(), holder, vaultAccount, sharesDestroyed, j_, std::nullopt, WaiveTransferFee::Yes);
        !isTesSuccess(ter))
        return ter;
    ...
    {
        if (auto const ter = removeEmptyHolding(view(), tx, holder, sharesDestroyed.asset(), j_); isTesSuccess(ter))
        {
            JLOG(j_.debug())  //
    ...
    if (assetsRecovered > beast::zero)
    {
        ...
        if (auto const ter =
                accountSend(view(), vaultAccount, account_, assetsRecovered, j_, std::nullopt, WaiveTransferFee::Yes);
            !isTesSuccess(ter))
            return ter;
