...
VaultDelete::doApply()
{
    ...
    if (auto ter = removeEmptyHolding(view(), ctx_.tx, vault->at(sfAccount), asset, j_); !isTesSuccess(ter))
        return ter;
    ...
    {
        if (auto const ter = removeEmptyHolding(view(), ctx_.tx, account_, MPTIssue(shareMPTID), j_);
            !isTesSuccess(ter))
        {
            // LCOV_EXCL_START
            JLOG(j_.error())  //
