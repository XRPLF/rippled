...
#include <xrpl/tx/transactors/payment/Payment.h>
    ...
    namespace xrpl
{
    TxConsequences Payment::makeTxConsequences(PreflightContext const& ctx)
    {
    ...
    STAmount getMaxSourceAmount(
        AccountID const& account, STAmount const& dstAmount, std::optional<STAmount> const& sendMax)
    {
        if (sendMax)
        {
            return *sendMax;
    ...
    bool Payment::checkExtraFeatures(PreflightContext const& ctx)
    {
    ...
    std::uint32_t Payment::getFlagsMask(PreflightContext const& ctx)
    {
        auto& tx = ctx.tx;
    ...
    NotTEC Payment::preflight(PreflightContext const& ctx)
    {
            ...
            (!mptDirect && maxSourceAmount.holds<MPTIssue>()))
        {
            JLOG(j.trace()) << "Malformed transaction: inconsistent issues: "
                            << dstAmount.getFullText() << " " << maxSourceAmount.getFullText()
    ...
    NotTEC Payment::checkPermission(ReadView const& view, STTx const& tx)
    {
