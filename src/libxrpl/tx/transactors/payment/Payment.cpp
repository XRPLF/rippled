...
Payment::preclaim(PreclaimContext const& ctx)
{
    ...
    else if (dstAcct->isFlag(lsfRequireDestTag) && !ctx.tx.isFieldPresent(sfDestinationTag))
    {
...
Payment::doApply()
{
    ...
    if (!dst)
    {
        ...
        dst->setFieldAmount(sfBalance, XRPAmount(beast::kZero));
    ...
    if (isDstMPT)
    {
        ...
        auto const issuance = MPTokenIssuance(view(), mptIssue);

        if (auto const ter = issuance.requireAuth(accountID_); !isTesSuccess(ter))
            return ter;

        if (auto const ter = issuance.requireAuth(dstAccountID); !isTesSuccess(ter))
            return ter;

        if (auto const ter = issuance.canTransfer(accountID_, dstAccountID); !isTesSuccess(ter))
            return ter;
        ...
        if (accountID_ != issuer && dstAccountID != issuer)
        {
            ...
            if (issuance.isAnyFrozen({accountID_, dstAccountID}))
                return tecLOCKED;

            // Get the rate for a payment between the holders.
            rate = issuance.transferRate();
        }
