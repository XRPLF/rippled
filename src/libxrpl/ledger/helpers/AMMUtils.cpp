...
<xrpl / ledger / helpers / AMMHelpers.h><xrpl / ledger / helpers / AMMUtils.h>
    ...
    namespace xrpl
{
    std::pair<STAmount, STAmount> ammPoolHolds(
        ReadView const& view,
    ...
    Expected<std::tuple<STAmount, STAmount, STAmount>, TER> ammHolds(
        ReadView const& view,
        SLE const& ammSle,
    ...
    STAmount ammLPHolds(
        ReadView const& view, SLE const& ammSle, AccountID const& lpAccount, beast::Journal const j)
    {
        return ammLPHolds(
            view,
    ...
    std::uint16_t getTradingFee(ReadView const& view, SLE const& ammSle, AccountID const& account)
    {
        ...
        if (ammSle.isFieldPresent(sfAuctionSlot))
        {
            auto const& auctionSlot =
                safe_downcast<STObject const&>(ammSle.peekAtField(sfAuctionSlot));
            // Not expired
    ...
    STAmount ammAccountHolds(
        ReadView const& view, AccountID const& ammAccountID, Issue const& issue)
    {
        if (isXRP(issue))
        {
    ...
    static TER deleteAMMTrustLines(
        Sandbox & sb,
        AccountID const& ammAccountID,
    ...
    TER deleteAMMAccount(Sandbox & sb, Issue const& asset, Issue const& asset2, beast::Journal j)
    {
        auto ammSle = sb.peek(keylet::amm(asset, asset2));
        if (!ammSle)
        {
            // LCOV_EXCL_START
            JLOG(j.error()) << "deleteAMMAccount: AMM object does not exist " << asset << " "
                            << asset2;
            return tecINTERNAL;
            // LCOV_EXCL_STOP
        }
    ...
    Expected<bool, TER> isOnlyLiquidityProvider(
        ReadView const& view, Issue const& ammIssue, AccountID const& lpAccount)
    {
    ...
    Expected<bool, TER> verifyAndAdjustLPTokenBalance(
        Sandbox & sb,
