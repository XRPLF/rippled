    ...
    Currency const& currency)
{
    ...
    auto sleRippleState = view.read(keylet::rippleState(account, issuer, currency));
    ...
    AccountID const& issuer)
{
    ...
    if (issuer != account)
    {
        ...
        auto const sle = view.read(keylet::rippleState(account, issuer, currency));
    ...
    AccountID const& issuer)
{
    ...
    {
        // Check if the issuer froze the line
        sle = view.read(keylet::rippleState(account, issuer, currency));
        if (sle && sle->isFlag((issuer > account) ? lsfHighFreeze : lsfLowFreeze))
            return true;
    }
    return false;
    ...
    beast::Journal j)
{
    ...
    auto const index = keylet::rippleState(issue.account, account, issue.currency);
    ...
    beast::Journal j)
{
    ...
    if (auto state = view.peek(keylet::rippleState(account, issue.account, issue.currency)))
    {
        STAmount final_balance = state->getFieldAmount(sfBalance);
...
requireAuth(ReadView const& view, Issue const& issue, AccountID const& account, AuthType authType)
{
    ...
    auto const trustLine = view.read(keylet::rippleState(account, issue.account, issue.currency));
...
canTransfer(ReadView const& view, Issue const& issue, AccountID const& from, AccountID const& to)
{
    ...
    auto const isRippleDisabled = [&](AccountID account) -> bool {
        ...
        auto const line = view.read(keylet::rippleState(account, issue));
        if (line)
        {
    ...
    beast::Journal journal)
{
    ...
    auto const index = keylet::rippleState(srcId, dstId, currency);
    ...
    beast::Journal journal)
{
    ...
    auto const line = view.peek(keylet::rippleState(accountID, issue));
    if (!line)
