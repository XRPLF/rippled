...
xrpLiquid(ReadView const& view, AccountID const& id, std::int32_t ownerCountAdj, beast::Journal j)
{
    ...
    auto const reserve = isPseudoAccount(sle)
        ...
        : view.fees().accountReserve(
              ownerCount, sponsoredOwnerCount, sponsoringOwnerCount, isAccountSponsored, sponsoringAccountCount);
    ...
    std::int32_t accountCountDelta)
{
    if (sponsorSle)
    {
        ...
        auto const sle =
            view.read(keylet::sponsor(sponsorSle->getAccountID(sfAccount), accSle->getAccountID(sfAccount)));
...
std::optional<AccountID>
getLedgerEntryReserveSponsorAccountID(std::shared_ptr<SLE const> const& sle, SF_ACCOUNT const& field)
{
...
std::shared_ptr<SLE>
getLedgerEntryReserveSponsor(ApplyView& view, std::shared_ptr<SLE> const& sle, SF_ACCOUNT const& field)
{
    ...
    beast::Journal journal)
{
    ...
    if (view.read(index))
        return tecDUPLICATE;
    auto const& sponsorAccountID = !isPseudoAccount(sleDst) ? getTxReserveSponsorAccountID(tx) : std::nullopt;
    ...
    if (auto const ret = checkInsufficientReserve(
            ...
            sleDst,
            priorBalance,
            sponsorAccountID ? view.peek(keylet::account(*sponsorAccountID)) : std::shared_ptr<SLE>(),
            1);
        !isTesSuccess(ret))
    ...
    std::optional<AccountID> holderID)
{
    ...
    if (!holderID)
    {
        ...
        {
            if (auto const ret = checkInsufficientReserve(view, tx, sleAcct, priorBalance, sponsor, 1);
                !isTesSuccess(ret))
                return ret;
        }
    ...
    WaiveTransferFee waiveFee)
{
    ...
    if (uSenderID == issuer || uReceiverID == issuer || issuer == noAccount())
    {
        ...
        auto const ter = rippleCreditIOU(view, uSenderID, uReceiverID, saAmount, false, sponsorAccount, j);
        if (ter != tesSUCCESS)
            return ter;
        saActual = saAmount;
        return tesSUCCESS;
    ...
    WaiveTransferFee waiveFee)
{
    ...
    for (auto const& r : receivers)
    {
        ...
        {
            // Direct send: redeeming IOUs and/or sending own IOUs.
            if (auto const ter = rippleCreditIOU(view, senderID, receiverID, amount, false, sponsorAccount, j))
                return ter;
            actual += amount;
        ...
        if (TER const terResult = rippleCreditIOU(view, issuer, receiverID, amount, true, sponsorAccount, j))
            return terResult;
    }

    if (senderID != issuer && takeFromSender)
    {
        if (TER const terResult = rippleCreditIOU(view, senderID, issuer, takeFromSender, true, sponsorAccount, j))
            return terResult;
    }
    ...
    WaiveTransferFee waiveFee)
{
    ...
    if (!saAmount.native())
    {
        ...
        return rippleSendIOU(view, uSenderID, uReceiverID, saAmount, saActual, j, sponsorAccount, waiveFee);
    }
    ...
    WaiveTransferFee waiveFee)
{
    ...
    if (!issue.native())
    {
        ...
        return rippleSendMultiIOU(view, senderID, issue, receivers, actual, j, sponsorAccount, waiveFee);
    }
    ...
    WaiveTransferFee waiveFee)
{
    return std::visit(
        [&]<ValidIssueType TIss>(TIss const& issue) {
            if constexpr (std::is_same_v<TIss, Issue>)
                return accountSendIOU(view, uSenderID, uReceiverID, saAmount, j, sponsorAcc, waiveFee);
            else
    ...
    WaiveTransferFee waiveFee)
{
    ...
    return std::visit(
        [&]<ValidIssueType TIss>(TIss const& issue) {
            if constexpr (std::is_same_v<TIss, Issue>)
                return accountSendMultiIOU(view, senderID, issue, receivers, j, sponsorAccount, waiveFee);
            else
    ...
    beast::Journal j)
{
    return std::visit(
        [&]<ValidIssueType TIss>(TIss const& issue) {
            if constexpr (std::is_same_v<TIss, Issue>)
            {
                return rippleCreditIOU(view, uSenderID, uReceiverID, saAmount, bCheckIssuer, std::nullopt, j);
            }
            else
            {
