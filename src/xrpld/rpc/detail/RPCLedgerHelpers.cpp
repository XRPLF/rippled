...
//
// Returns a json::ValueType::ObjectValue.  If there was an error, it will be in that
...
getOrAcquireLedger(RPC::JsonContext const& context)
{
    ...
    if (hasHash)
    {
        auto const& jsonHash = context.params.get(jss::ledger_hash, json::ValueType::NullValue);
    ...
    {
        auto const& jsonIndex = context.params.get(jss::ledger_index, json::ValueType::NullValue);
