...
private:
    ...
    parseManifest(json::Value const& jvParams)
    {
        if (jvParams.size() == 1)
        {
            json::Value jvRequest(json::ValueType::ObjectValue);
    ...
    parseDepositAuthorized(json::Value const& jvParams)
    {
        ...
        {
            jvRequest[jss::credentials] = json::Value(json::ValueType::ArrayValue);
    ...
    parseJson2(json::Value const& jvParams)
    {
        ...
        if (validParse && isValidJson2(jv))
        {
            if (jv.isObject())
            {
                json::Value jv1{json::ValueType::ObjectValue};
            ...
            // else jv.isArray()
            json::Value jv1{json::ValueType::ArrayValue};
    ...
    parseGatewayBalances(json::Value const& jvParams)
    {
        ...
        if (index < size)
        {
            json::Value& hotWallets = (jvRequest["hotwallet"] = json::ValueType::ArrayValue);
            while (index < size)
    ...
    beast::Journal j)
{
    ...
    json::Value jvRpcParams(json::ValueType::ArrayValue);

    for (int i = 1; i != args.size(); i++)
        jvRpcParams.append(args[i]);

    retParams = json::Value(json::ValueType::ObjectValue);
    ...
    std::unordered_map<std::string, std::string> const& headers)
{
    ...
    try
    {
        json::Value jvRpc = json::Value(json::ValueType::ObjectValue);
        ...
        else
        {
            ...
            json::Value jvParams(json::ValueType::ArrayValue);
