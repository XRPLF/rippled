    ...
    json::Value const& jv)
{
    ...
    json::Value jr(json::ValueType::ObjectValue);
...
{
    json::Value sub{json::ValueType::ObjectValue};
    sub["code"] = code;
    sub["message"] = std::move(message);
    json::Value r{json::ValueType::ObjectValue};
    r["error"] = sub;
    return r;
}
    ...
    std::string_view user)
{
    ...
    json::Value reply(batch ? json::ValueType::ArrayValue : json::ValueType::ObjectValue);
    ...
    for (unsigned i = 0; i < size; ++i)
    {
        ...
        {
            role = requestRole(required, port, json::ValueType::ObjectValue, remoteIPAddress, user);
        }
        ...
        if (!batch)
        {
            ...
            if (!params)
            {
                params = json::Value(json::ValueType::ObjectValue);
            }
