...
doAccountCurrencies(RPC::JsonContext& context)
{
    ...
    json::Value& sendCurrencies = (result[jss::send_currencies] = json::ValueType::ArrayValue);
    for (auto const& c : send)
        sendCurrencies.append(to_string(c));

    json::Value& recvCurrencies = (result[jss::receive_currencies] = json::ValueType::ArrayValue);
