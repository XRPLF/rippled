    ...
    namespace xrpl
{
    ...
    public:
        ...
        {
            auto const srcAcct =
                (prevStep != nullptr) ? prevStep->directStepSrcAcct() : std::nullopt;
    ...
    template <class TIn, class TOut, class TDerived>
    std::pair<std::optional<Quality>, DebtDirection>
    BookStep<TIn, TOut, TDerived>::qualityUpperBound(ReadView const& v, DebtDirection prevStepDir)
        const
    {
    ...
    template <class TIn, class TOut, class TDerived>
    std::pair<std::optional<QualityFunction>, DebtDirection>
    BookStep<TIn, TOut, TDerived>::getQualityFunc(ReadView const& v, DebtDirection prevStepDir)
        const
    {
        auto const dir = this->debtDirection(v, StrandDirection::forward);
    ...
    template <class TIn, class TOut, class TDerived>
    std::uint32_t BookStep<TIn, TOut, TDerived>::offersUsed() const
    {
        return offersUsed_;
    }
    ...
    BookStep<TIn, TOut, TDerived>::forEachOffer(
        PaymentSandbox & sb, ApplyView & afView, DebtDirection prevStepDir, Callback & callback)
        const
    {
        // Charge the offer owner, not the sender
        // Charge a fee even if the owner is the same as the issuer
        ...
        auto execOffer = [&](auto& offer) {
            ...
            TAmounts stpAmt{
                mulRatio(ofrAmt.in, ofrInRate, QUALITY_ONE, /*roundUp*/ true), ofrAmt.out};
        ...
        TOut const& ownerGives) const
    {
        ...
        // stepAmt is a transfer fee that goes to book_.in.account
        {
            auto const dr = offer.send(
                sb, book_.in.account, offer.owner(), toSTAmount(ofrAmt.in, book_.in), j_);
            if (!isTesSuccess(dr))
        ...
        // stepAmt is a transfer fee that goes to book_.out.account
        {
            auto const cr = offer.send(
                sb, offer.owner(), book_.out.account, toSTAmount(ownerGives, book_.out), j_);
            if (!isTesSuccess(cr))
    ...
    template <class TIn, class TOut, class TDerived>
    std::optional<AMMOffer<TIn, TOut>> BookStep<TIn, TOut, TDerived>::getAMMOffer(
        ReadView const& view, std::optional<Quality> const& clobQuality) const
    {
        if (ammLiquidity_)
    ...
    template <class TIn, class TOut, class TDerived>
    std::optional<std::variant<Quality, AMMOffer<TIn, TOut>>> BookStep<TIn, TOut, TDerived>::tip(
    ...
    template <class TIn, class TOut, class TDerived>
    auto BookStep<TIn, TOut, TDerived>::tipOfferQuality(ReadView const& view) const
        -> std::optional<std::pair<Quality, OfferType>>
    {
    ...
    template <class TIn, class TOut, class TDerived>
    std::optional<QualityFunction> BookStep<TIn, TOut, TDerived>::tipOfferQualityF(
        ReadView const& view) const
    {
        auto const res = tip(view);
        if (!res)
    ...
    template <class TIn, class TOut, class TDerived>
    std::pair<TIn, TOut> BookStep<TIn, TOut, TDerived>::revImp(
        PaymentSandbox & sb,
        ApplyView & afView,
        boost::container::flat_set<uint256> & ofrsToRm,
        TOut const& out)
    ...
    template <class TIn, class TOut, class TDerived>
    std::pair<TIn, TOut> BookStep<TIn, TOut, TDerived>::fwdImp(
        PaymentSandbox & sb,
        ApplyView & afView,
        boost::container::flat_set<uint256> & ofrsToRm,
        TIn const& in)
    ...
    template <class TIn, class TOut, class TDerived>
    std::pair<bool, EitherAmount> BookStep<TIn, TOut, TDerived>::validFwd(
        PaymentSandbox & sb, ApplyView & afView, EitherAmount const& in)
    {
        if (!cache_)
        {
    ...
    template <class TIn, class TOut, class TDerived>
    TER BookStep<TIn, TOut, TDerived>::check(StrandContext const& ctx) const
    {
    ...
    bookStepEqual(Step const& step, xrpl::Book const& book)
    {
        ...
        if (inXRP && !outXRP)
            return equalHelper<XRPAmount, IOUAmount, BookPaymentStep<XRPAmount, IOUAmount>>(
                step, book);
        if (!inXRP && outXRP)
            return equalHelper<IOUAmount, XRPAmount, BookPaymentStep<IOUAmount, XRPAmount>>(
                step, book);
        if (!inXRP && !outXRP)
            return equalHelper<IOUAmount, IOUAmount, BookPaymentStep<IOUAmount, IOUAmount>>(
                step, book);
        return false;
    }
    }  // namespace test
    ...
    template <class TIn, class TOut>
    static std::pair<TER, std::unique_ptr<Step>> make_BookStepHelper(
    ...
    std::pair<TER, std::unique_ptr<Step>> make_BookStepII(
        StrandContext const& ctx, Issue const& in, Issue const& out)
    {
    ...
    std::pair<TER, std::unique_ptr<Step>> make_BookStepXI(
        StrandContext const& ctx, Issue const& out)
    {
