...
AMMOffer<TIn, TOut>::limitOut(
    TAmounts<TIn, TOut> const& offerAmt,
    TOut const& limit,
    bool roundUp) const
{
    ...
    if (ammLiquidity_.multiPath())
    {
        ...
        return quality().ceil_out_strict(offerAmt, limit, roundUp);
    }
    ...
    bool roundUp) const
{
    ...
    if (ammLiquidity_.multiPath())
    {
        if (auto const& rules = getCurrentTransactionRules();
            ...
            return quality().ceil_in_strict(offerAmt, limit, roundUp);

        return quality().ceil_in(offerAmt, limit);
    }
