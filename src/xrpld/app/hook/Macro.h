...
#define DEFINE_HOOK_FUNCNARG(R, F)                                    \
    ...
    WasmEdge_String hook_api::WasmFunctionName##F =                   \
        WasmEdge_StringCreateByCString(#F);                           \
    R hook_api::F(                                                    \
        hook::HookContext& hookCtx, WasmEdge_MemoryInstanceContext& memoryCtx)
...
#define HOOK_SETUP()                                            \
    ...
    [[maybe_unused]] const uint64_t memory_length =             \
        WasmEdge_MemoryInstanceGetPageSize(&memoryCtx) * WasmEdge_kPageSize;
...
#define RETURN_HOOK_TRACE(read_ptr, read_len, t)                               \
    {                                                                          \
        ...
        {                                                                      \
            JLOG(j.trace()) << "HookTrace[" << HC_ACC() << "]: " << t;         \
        }                                                                      \
        ...
        {                                                                      \
            JLOG(j.trace())                                                    \
                << "HookTrace[" << HC_ACC() << "]: "                           \
                << std::string_view(                                           \
                       (const char*)(memory + read_ptr), (size_t)rl)           \
                << " " << t;                                                   \
        }                                                                      \
        return 0;                                                              \
    }
