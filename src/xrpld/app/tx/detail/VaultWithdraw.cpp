...
VaultWithdraw::doApply()
{
    ...
    if (auto const ter =
            accountSend(view(), account_, vaultAccount, sharesRedeemed, j_, sponsor, WaiveTransferFee::Yes);
        !isTesSuccess(ter))
        return ter;
    ...
    {
        if (auto const ter = removeEmptyHolding(view(), ctx_.tx, account_, sharesRedeemed.asset(), j_);
            isTesSuccess(ter))
        {
            JLOG(j_.debug())  //
