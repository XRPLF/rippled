...
private:
    ...
    struct SubAccountHistoryIndex
    {
        ...
        SubAccountHistoryIndex(AccountID const& accountId)
            : accountId_(accountId), stopHistorical_(false)
        {
        }
    };
...
NetworkOPsImp::switchLastClosedLedger(std::shared_ptr<Ledger const> const& newLCL)
{
    ...
    s.set_ledgerhashprevious(newLCL->header().parentHash.begin(), uint256::size());
    s.set_ledgerhash(newLCL->header().hash.begin(), uint256::size());
    ...
    registry_.getJobQueue().addJob(
        jtCLIENT_ACCT_HIST, "HistTxStream", [this, dbType = databaseType, subInfo]() {
            ...
            auto getMoreTxns = [&](std::uint32_t minLedger,
                ...
                {
                    case Sqlite: {
                        auto db = dynamic_cast<SQLiteDatabase*>(&registry_.getRelationalDatabase());
...
NetworkOPsImp::subServer(InfoSub::ref isrListener, Json::Value& jvResult, bool admin)
{
    ...
    beast::rngfill(uRandom.begin(), uint256::size(), crypto_prng());
