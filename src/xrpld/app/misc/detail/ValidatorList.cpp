    ...
    beast::Journal j)
{
    ...
    switch (effectiveVersion)
    {
        ...
        case 2: {
            json::Value blobs(json::ValueType::ArrayValue);

            auto add = [&blobs,
                        &outerManifest = pubCollection.rawManifest](PublisherList const& pubList) {
                auto& blob = blobs.append(json::ValueType::ObjectValue);
        ...
        default:
            ...
            value = json::ValueType::NullValue;
    }
...
ValidatorList::getJson() const
{
    ...
    {
        auto& x = (res[jss::validator_list] = json::ValueType::ObjectValue);
    ...
    json::Value& jLocalStaticKeys = (res[jss::local_static_keys] = json::ValueType::ArrayValue);

    for (auto const& key : localPublisherList_.list)
        jLocalStaticKeys.append(toBase58(TokenType::NodePublic, key));

    // Publisher lists
    json::Value& jPublisherLists = (res[jss::publisher_lists] = json::ValueType::ArrayValue);
    for (auto const& [publicKey, pubCollection] : publisherLists_)
    {
        json::Value& curr = jPublisherLists.append(json::ValueType::ObjectValue);
        ...
        auto appendList = [](PublisherList const& publisherList, json::Value& target) {
            ...
            json::Value& keys = (target[jss::list] = json::ValueType::ArrayValue);
        ...
        json::Value remaining(json::ValueType::ArrayValue);
        for (auto const& [sequence, future] : pubCollection.remaining)
        {
            using namespace std::chrono_literals;

            (void)sequence;
            json::Value& r = remaining.append(json::ValueType::ObjectValue);
            appendList(future, r);
    ...
    json::Value& jValidatorKeys = (res[jss::trusted_validator_keys] = json::ValueType::ArrayValue);
    ...
    // signing keys
    json::Value& jSigningKeys = (res[jss::signing_keys] = json::ValueType::ObjectValue);
    ...
    {
        json::Value& jNegativeUNL = (res[jss::NegativeUNL] = json::ValueType::ArrayValue);
