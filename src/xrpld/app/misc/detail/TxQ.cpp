    ...
    std::shared_ptr<STTx const> const& tx)
{
    ...
    {
        auto const& replacedTx = (*replacedTxIter)->second;
        ...
        if (replacedTx.parentTx)
        {
            TxQApplyImpl::removeBatchTx(byAccount_, byFee_, *replacedTx.parentTx);
            return;
        }

        // If the transaction we're holding replaces a transaction in the
        // queue, remove the transaction that is being replaced.
        auto deleteIter = byFee_.iterator_to(replacedTx);
