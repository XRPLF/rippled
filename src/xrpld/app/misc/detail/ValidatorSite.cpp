...
ValidatorSite::getJson() const
{
    ...
    json::Value jrr(json::ValueType::ObjectValue);
    json::Value& jSites = (jrr[jss::validator_sites] = json::ValueType::ArrayValue);
    {
        std::scoped_lock const lock{sites_mutex_};
        for (Site const& site : sites_)
        {
            json::Value& v = jSites.append(json::ValueType::ObjectValue);
