...
private:
    ...
    generateServerInfo()
    {
        ...
        // Populate the DebugCounters block (db KB, hit rates, tree-node cache/track
        // sizes, nodestore counts). getDebugCounters() was implemented but never
        // called, so this fixed-offset block shipped all-zero — blinding the
        // per-node memory-driver telemetry (treenode_track_size etc.).
