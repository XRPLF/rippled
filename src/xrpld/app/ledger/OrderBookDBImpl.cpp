...
OrderBookDBImpl::update(std::shared_ptr<ReadView const> const& ledger)
{
    ...
    // Build/rebuild PayGraph against *this* ledger (the one just scanned).
    // Must run after allBooks_ is swapped in.  Fixes RpcNotReady when
    // path_find arrived before updateAll could build the graph, and avoids
    // building against a stale validated ledger that lacks these offers.
    try
    {
        registry_.get().getPathRequestManager().signalOrderBookReady(ledger);
    }
    catch (...)
    {
