    ...
    namespace xrpl
{
    ...
    std::unique_ptr<OrderBookDB> make_OrderBookDB(
        ServiceRegistry & registry, OrderBookDBConfig const& config)
    {
        return std::make_unique<OrderBookDBImpl>(registry, config);
    }

    void OrderBookDBImpl::setup(std::shared_ptr<ReadView const> const& ledger)
    {
    ...
    void OrderBookDBImpl::update(std::shared_ptr<ReadView const> const& ledger)
    {
        if (pathSearchMax_ == 0)
    ...
    void OrderBookDBImpl::addOrderBook(Book const& book)
    {
    ...
    std::vector<Book> OrderBookDBImpl::getBooksByTakerPays(
        Issue const& issue, std::optional<uint256> const& domain)
    {
        std::vector<Book> ret;
    ...
    int OrderBookDBImpl::getBookSize(Issue const& issue, std::optional<uint256> const& domain)
    {
        std::lock_guard sl(mLock);
    ...
    bool OrderBookDBImpl::isBookToXRP(Issue const& issue, std::optional<Domain> domain)
    {
        std::lock_guard sl(mLock);
        if (domain)
    ...
    BookListeners::pointer OrderBookDBImpl::makeBookListeners(Book const& book)
    {
        std::lock_guard sl(mLock);
    ...
    BookListeners::pointer OrderBookDBImpl::getBookListeners(Book const& book)
    {
    ...
    void OrderBookDBImpl::processTxn(
