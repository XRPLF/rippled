#!/usr/bin/env python3
"""Map changed lines in a PR to feature-interaction graph nodes.

Reads a graph built by `build_graph.py` and a git diff, and reports which
feature and resource nodes the diff touches. Exact macro and definition matches
are reported as `span`; whole implementation-file matches are `file`. Changes
outside recorded spans remain `off_span_changes` rather than being attributed
to every node in that file. The mapper also records newly introduced field,
flag, and amendment tokens for the selector.
"""
