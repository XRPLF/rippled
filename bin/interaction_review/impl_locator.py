...
A transactor is matched to source using three rules:
...
2. **Symbol** — a file defining `Name::method` at column 0, or declaring
   `class Name`. Stem and symbol matches are combined.
3. **Alias** — a `using Name = Other;` declaration, resolved by re-running rules
   1 and 2 against `Other`.

Unresolved transactors fail the graph build. The override file handles layouts
these rules cannot express.
"""
