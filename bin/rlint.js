...
var book_key = function(book) {
  return (book.taker_pays.currency + ":" + book.taker_pays.issuer + ":" +
          book.taker_gets.currency + ":" + book.taker_gets.issuer);
};

var book_key_cross = function(book) {
  return (book.taker_gets.currency + ":" + book.taker_gets.issuer + ":" +
          book.taker_pays.currency + ":" + book.taker_pays.issuer);
};
...
var ledger_verify = function(ledger) {
  ...
  Object.keys(books).forEach(function(key) {
    ...
    if (book && book_cross && !book_cross.done) {
      var book_cross_quality_inverted = Amount.from_json("1.0/1/1").divide(
          book_cross.quality,
      );
      ...
      if (book_cross_quality_inverted.compareTo(book.quality) >= 0) {
        // Crossing books
        console.log(
            "crossing: #%s :: %s :: %s :: %s :: %s :: %s :: %s",
            ledger.ledger_index,
            key,
            book.quality.to_text(),
            book_cross.quality.to_text(),
            book_cross_quality_inverted.to_text(),
            book.index,
            book_cross.index,
        );
      }
  ...
  ledger.accountState.forEach(function(entry) {
    if (entry.LedgerEntryType === "Offer") {
      ...
      if (entry.Flags & (0x10000 | 0x40000)) {
        counts[entry.LowLimit.issuer] =
            (counts[entry.LowLimit.issuer] || 0) + 1;
      }

      if (entry.Flags & (0x20000 | 0x80000)) {
        counts[entry.HighLimit.issuer] =
            (counts[entry.HighLimit.issuer] || 0) + 1;
      }
  ...
  Object.keys(accounts).forEach(function(account) {
    if (!("OwnerCount" in accounts[account])) {
      console.log(
          "%s: bad entry : %s",
          account,
          JSON.stringify(accounts[account], undefined, 2),
      );
...
var usage =
    function() { console.log("rlint.js _websocket_ip_ _websocket_port_ "); };
          ...
          .once("ledger_closed",
                function(m) {
                  ...
                  if (process.argv.length === 5) {
                    ...
                    async.whilst(
                        function() {
                          return ledger_start <= ledger_cursor &&
                                 ledger_cursor <= ledger_end;
                        },
                        function(callback) {
