...
var httpd_response = function(res, opts) {
  ...
  res.end(
      "<HTML>" +
          ...
          //      + "<LI><A
          //      HREF=\""+test+"\">rHb9CJAWyB4rj91VRWn96DkukG4bwdtyTh</A>"
          '<LI><A HREF="/ledger">ledger</A>' +
          "</UL>" + (opts.body || "") + "<HR><PRE>" + (opts.url || "") +
          "</PRE>" +
          "</BODY>" +
          "</HTML>",
  );
...
var html_link = function(generic) {
  return ('<A HREF="' + build_uri({type : "account", account : generic}) +
          '">' + generic + "</A>");
};
    ...
    if ("Offer" === obj.LedgerEntryType) {
      if ("NewFields" in obj) {
        if ("TakerGets" in obj.NewFields)
          obj.NewFields.TakerGets = rewrite_type(
              "amount",
              obj.NewFields.TakerGets,
              opts,
          );

        if ("TakerPays" in obj.NewFields)
          obj.NewFields.TakerPays = rewrite_type(
              "amount",
              obj.NewFields.TakerPays,
              opts,
          );
      }
    }
...
var rewrite_object = function(obj, opts) {
  ...
  if ("TransactionType" in obj) {
    ...
    out.meta.AffectedNodes = out.meta.AffectedNodes.map(function(node) {
      var kind = "CreatedNode" in node    ? "CreatedNode"
                 : "ModifiedNode" in node ? "ModifiedNode"
                 : "DeletedNode" in node  ? "DeletedNode"
                                          : undefined;
          ...
          opts.remote.request_tx(tx_hash)
              .on("success",
                  function(m) {
                    ...
                    m.meta.AffectedNodes
                        .filter(function(n) {
                          // console.log("augment_object: ", JSON.stringify(n));
                          // if (n.ModifiedNode)
                          // console.log("augment_object: %s %s %s %s %s %s/%s",
                          // 'ModifiedNode' in n, n.ModifiedNode &&
                          // (n.ModifiedNode.LedgerEntryType === 'AccountRoot'),
                          // n.ModifiedNode && n.ModifiedNode.FinalFields &&
                          // (n.ModifiedNode.FinalFields.Account ===
                          // obj.node.Account), Object.keys(n)[0],
                          // n.ModifiedNode && (n.ModifiedNode.LedgerEntryType),
                          // obj.node.Account, n.ModifiedNode &&
                          // n.ModifiedNode.FinalFields &&
                          // n.ModifiedNode.FinalFields.Account); if
                          // ('ModifiedNode' in n &&
                          // n.ModifiedNode.LedgerEntryType === 'AccountRoot')
                          // {
                          ...
                          return ("ModifiedNode" in n &&
                                  n.ModifiedNode.LedgerEntryType ===
                                      "AccountRoot" &&
                                  n.ModifiedNode.FinalFields &&
                                  n.ModifiedNode.FinalFields.Account ===
                                      obj.node.Account);
                        ...
                        .forEach(function(n) {
                          ...
                          console.log("augment_object: next: %s %s", tx_hash,
                                      tx_ledger);
                        });

                    callback();
                  })
              .on("error", function(m) { callback(m); })
              .request();
        },
        function(err) {
          if (err) {
...
if (process.argv.length < 4 || process.argv.length > 7) {
            ...
            remote
                ...
                .on("success", function(m) {
                  ...
                  httpd_response(res, {
                    ...
                    url : _url,
                    body : "<PRE>" +
                               JSON.stringify(rewrite_object(m, self.base),
                                              undefined, 2) +
                               "</PRE>",
                  });
                });
  ...
  server.listen(
      port, ip, undefined,
      function() { console.log("Listening at: http://%s:%s", ip, port); });
}
