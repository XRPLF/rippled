...
String(fs.readFileSync(filenameProto)).split("\n").forEach(function(line) {
  ...
  if (!line.length || line.slice(0, 2) === "//" || line.slice(-1) !== ")")
    return;

  var tmp = line.slice(0, -1).split("("), type = tmp[0],
      opts = tmp[1].split(",");

  if (type === "TYPE")
    types[opts[1]] = [ opts[0], +opts[2] ];
  else if (type === "FIELD")
    fields[opts[0]] = [ types[opts[1]][0], +opts[2] ];
});
...
Object.keys(types).forEach(function(type) {
  output.push(
      pad("ST." + types[type][0] + ".id", 25) + " = " + types[type][1] + ";",
  );
});
output.push("");
...
String(fs.readFileSync(filenameTxFormats)).split("\n").forEach(function(line) {
  ...
  if (d_match)
    sections.push((current = [ d_match[1], ttConsts[d_match[2]] ]));
  else if (s_match)
    current.push([ s_match[1], s_match[2] ]);
});
...
sections.forEach(function(section) {
  var name = section.shift(), ttid = section.shift();
