...
The Go bindings are generated by the CMake target `go_protobuf`. To regenerate
them for the changed .proto files, run `cmake --build . --target go_protobuf`
from the build directory. Use Nix to ensure the correct toolchain versions are
used (`nix develop` from the repository root).

The generated files are committed to the repository and can be imported by
external clients as a Go module.

To update the dependencies used by the Go module itself, run `go get -u ./...`
followed by `go mod tidy` from the `proto/org/xrpl/rpc/v1` directory, and
commit any changes to the `go.mod` and `go.sum` files.
