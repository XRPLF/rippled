...
{
    // next_ is the `expected` operand of the CAS below and must be written
    // before `this` is published, so it cannot be a member initializer.
    next_ = gCountedObjects.head_.load(
        std::memory_order::relaxed);  // NOLINT(cppcoreguidelines-prefer-member-initializer)
