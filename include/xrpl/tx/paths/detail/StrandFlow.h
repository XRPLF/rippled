    ...
    namespace xrpl
{
    ...
    /// @cond INTERNAL
    inline std::optional<Quality> qualityUpperBound(ReadView const& v, Strand const& strand)
    {
    ...
    */
        ...
        path::detail::FlowDebugInfo* flowDebugInfo = nullptr)
    {
        ...
        std::optional<TInAmt> const sendMax = (sendMaxST && sendMaxInit >= beast::zero)
        ...
        while (remainingOut > beast::zero && (!remainingIn || *remainingIn > beast::zero))
        {
            ...
            for (size_t strandIndex = 0, sie = activeStrands.size(); strandIndex != sie;
                 ++strandIndex)
            {
