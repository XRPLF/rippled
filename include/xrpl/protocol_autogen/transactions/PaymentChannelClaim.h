...
public:
    PaymentChannelClaimBuilder(SF_ACCOUNT::type::value_type account,
                     ...
                     std::decay_t<typename SF_UINT256::type::value_type> const& channel)
        : TransactionBuilderBase<PaymentChannelClaimBuilder>(account, sequence, fee, signingPubKey, ttPAYCHAN_CLAIM)
    {
        setChannel(channel);
    }
    ...
    build()
    {
        return PaymentChannelClaim(STTx(std::move(object_)));
    }
};
