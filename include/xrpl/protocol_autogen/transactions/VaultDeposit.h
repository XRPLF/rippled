...
public:
     ...
     */
    explicit VaultDeposit(STTx const& tx)
        : TransactionBase(tx)
    {
...
public:
    VaultDepositBuilder(SF_ACCOUNT::type::value_type account,
        ...
        : TransactionBuilderBase<VaultDepositBuilder>(account, sequence, fee, signingPubKey, ttVAULT_DEPOSIT)
    {
        setVaultID(vaultID);
        setAmount(amount);
    }

    VaultDepositBuilder(STTx const& tx)
    {
    ...
    /**
     * Build and return the completed VaultDeposit wrapper.
    ...
    build()
    {
        return VaultDeposit(STTx(std::move(object_)));
    }
};
