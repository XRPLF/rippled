...
/**
 * @brief Selects which ConfidentialMPTHolderKeyUpdate mode a proof is for.
 */
enum class HolderKeyUpdateMode { Rotation, Recovery };

/**
 * @brief Verifies the zero-knowledge proof for a ConfidentialMPTHolderKeyUpdate
 * transaction.
 *
 * In Rotation mode, this must verify a Schnorr proof of knowledge of the new
 * ElGamal private key, plus a Compact Pedersen proof that
 * newSpendingCiphertext/newInboxCiphertext encrypt the same amounts as
 * oldSpendingCiphertext/oldInboxCiphertext under the new public key. In
 * Recovery mode, this must verify only the Schnorr proof of knowledge of the
 * new private key;
 *
 * @note TODO: the mpt-crypto library does not yet expose a Compact
 * Pedersen equality-proof verifier for key rotation. This is currently
 * a placeholder that always returns tesSUCCESS; real verification will be
 * wired in once that crypto-side work lands.
 *
 * @param mode                  Rotation or Recovery.
 * @param proof                 The full proof blob.
 * @param newPubKey             The holder's new ElGamal public key.
 * @param oldSpendingCiphertext The holder's current spending balance ciphertext
 *                              (Rotation mode only).
 * @param oldInboxCiphertext    The holder's current inbox balance ciphertext
 *                              (Rotation mode only).
 * @param newSpendingCiphertext The holder's new spending balance ciphertext,
 *                              re-encrypted under newPubKey (Rotation mode only).
 * @param newInboxCiphertext    The holder's new inbox balance ciphertext,
 *                              re-encrypted under newPubKey (Rotation mode only).
 * @param contextHash           The context hash binding the proof.
 * @return tesSUCCESS if the proof is valid, or an error code otherwise.
 */
