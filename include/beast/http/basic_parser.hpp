...
/** Parser for producing HTTP requests and responses.
    ...
    signatures which can be present in the derived class are:
    ...
    * int on_headers(error_code& ec)
        ...
        The return value from this callback is special, these
        are the meanings:

        0 : The parser should expect a body
        1 : The parser should skip the body. For example, this is
            used when sending a response to a HEAD request.
        2 : The parser should skip ths body, this is an
    ...
    * void on_complete(error_code& ec)
    ...
    with no compilation error. The default behavior of on_body when
    the derived class does not provide the member, is to specify that
